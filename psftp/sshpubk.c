...
{
	if		( ptrlen_eq_string( name, "ssh-rsa" ) )				{ return &ssh_rsa; }
	else if ( ptrlen_eq_string( name, "ssh-dss" ) )				{ return &ssh_dss; }
	else if ( ptrlen_eq_string( name, "ecdsa-sha2-nistp256" ) )	{ return &ssh_ecdsa_nistp256; }
	else if ( ptrlen_eq_string( name, "ecdsa-sha2-nistp384" ) )	{ return &ssh_ecdsa_nistp384; }
	else if ( ptrlen_eq_string( name, "ecdsa-sha2-nistp521" ) )	{ return &ssh_ecdsa_nistp521; }
	else if ( ptrlen_eq_string( name, "ssh-ed25519" ) )			{ return &ssh_ecdsa_ed25519; }
	else														{ return NULL; }
}
...
ssh2_userkey *ssh2_load_userkey( const Filename *filename, const char *passphrase, const char **errorstr )
{
	...
	*/
	{
		...
		else
		{
			...
			put_string( macdata, public_blob->s, public_blob->len );
			put_string( macdata, private_blob->s, private_blob->len );
			free_macdata = true;
		}
	...
	*/
	...
	ret->comment = comment;
	ret->key = ssh_key_new_priv( alg, ptrlen_from_strbuf( public_blob ), ptrlen_from_strbuf( private_blob ) );
	if ( !ret->key )
	{
		sfree( ret );
		ret = NULL;
...
error:
	...
	if ( mac )			{ sfree( mac ); }
	if ( public_blob )	{ strbuf_free( public_blob ); }
	if ( private_blob )	{ strbuf_free( private_blob ); }
	if ( errorstr )		{ *errorstr = error; }

	return ret;
}

bool rfc4716_loadpub( FILE *fp, char **algorithm, BinarySink *bs, char **commentptr, const char **errorstr )
{
	const char *error;
	...
	while ( 1 )
	{
		...
		*colon = '\0';
		...
		else if ( !strcmp( line, "Subject" ) || !strncmp( line, "x-", 2 ) )
		{
...
error:
	sfree( line );
	sfree( comment );
	if ( pubblob )	{ strbuf_free( pubblob ); }
	if ( errorstr )	{ *errorstr = error; }

	return false;
}

bool openssh_loadpub( FILE *fp, char **algorithm, BinarySink *bs, char **commentptr, const char **errorstr )
{
	const char *error;
	char *line, *base64;
	char *comment = NULL;
	...
	*/
	alglen = strlen( line );
	if ( pubbloblen < alglen + 4 || GET_32BIT_MSB_FIRST( pubblob ) != alglen || 0 != memcmp( pubblob + 4, line, alglen ) )
	{
...
bool ssh2_userkey_loadpub( const Filename *filename, char **algorithm, BinarySink *bs, char **commentptr, const char **errorstr )
{
	...
	* we'll be asked to read a public blob from one of those. */
	...
	/* Read the first header line which contains the key type. */
	if ( !read_header( fp, header ) || ( 0 != strcmp( header, "PuTTY-User-Key-File-2" ) && 0 != strcmp( header, "PuTTY-User-Key-File-1" ) ) )
	{
...
static char *ssh2_pubkey_openssh_str_internal( const char *comment, const void *v_pub_blob, int pub_len )
{
	...
	buffer = snewn( alg.len + 4 * ( ( pub_len + 2 ) / 3) + ( comment ? strlen( comment ) : 0 ) + 3, char );
...
void base64_encode_buf( const unsigned char *data, int datalen, char *out )
{
	...
	{
		n = ( datalen < 3 ? datalen : 3 );
		base64_encode_atom( data, n, out + len );
		data += n;
		len += 4;
		datalen -= n;
	}
...
void get_key_info( ssh_key *data, char **algorithm_str, int *key_size, char **md5_fingerprint_str, char **sha256_fingerprint_str )
{
	...
	if ( md5_fingerprint_str == NULL || sha256_fingerprint_str == NULL )
	{
		return;
	}
	...
	ptrlen blob_len = ptrlen_from_strbuf( blob );

	*algorithm_str = NULL;
	*md5_fingerprint_str = snewn( 53, char );		// "MD5: " + 48
	*sha256_fingerprint_str = snewn( 54, char );	// "SHA-256: " + ceil(32/3)*4+1

	// The fingerprint hash itself is always just the MD5 of the blob.
	hash_simple( &ssh_md5, blob_len, digest );
	memcpy( *md5_fingerprint_str, "MD5: ", 5 );
	for ( i = 0; i < 16; ++i )
	{
		sprintf( ( *md5_fingerprint_str + 5 ) + ( i * 3 ), "%02x%s", digest[ i ], i == 15 ? "" : ":" );
	}

	hash_simple( &ssh_sha256, blob_len, digest );
	memcpy( *sha256_fingerprint_str, "SHA-256: ", 9 );
	base64_encode_buf( digest, 32, ( *sha256_fingerprint_str + 9 ) );

	// Identify the key algorithm, if possible.
	BinarySource_BARE_INIT_PL( src, blob_len );
	algname = get_string( src );
	if ( !get_err( src ) )
	{
		alg = find_pubkey_alg_len( algname );

		*key_size = ssh_key_public_bits( alg, blob_len );
		*algorithm_str = dupprintf( "%.*s", PTRLEN_PRINTF( algname ) );
	}
...
static int key_type_fp( FILE *fp )
{
	...
	buf[ i ] = '\0';
	if ( !memcmp( buf, rsa_signature, sizeof( rsa_signature ) - 1 ) )		{ return SSH_KEYTYPE_SSH1; }
	if ( !memcmp( buf, public_std_sig, sizeof( public_std_sig ) - 1 ) )		{ return SSH_KEYTYPE_SSH2_PUBLIC_RFC4716; }
	if ( !memcmp( buf, putty2_sig, sizeof( putty2_sig ) - 1 ) )				{ return SSH_KEYTYPE_SSH2; }
	if ( !memcmp( buf, openssh_new_sig, sizeof( openssh_new_sig ) - 1 ) )	{ return SSH_KEYTYPE_OPENSSH_NEW; }
	if ( !memcmp( buf, openssh_sig, sizeof( openssh_sig ) - 1 ) )			{ return SSH_KEYTYPE_OPENSSH_PEM; }
	if ( !memcmp( buf, sshcom_sig, sizeof( sshcom_sig ) - 1 ) )				{ return SSH_KEYTYPE_SSHCOM; }
	...
	if ( ( p = buf + strcspn( buf, " " ),
		 ...
		 ( p = p + 1 + strspn( p + 1, "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz+/=" ),
		  *p == ' ' || *p == '\n' || !*p ) )
	{
...
/*
 * Convert the type word to a string, for `wrong type' error messages.
 */
