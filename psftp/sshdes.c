 ...
 */
static __inline uint32_t rol( uint32_t x, unsigned c )
{
	return ( x << ( 31 & c ) ) | ( x >> ( 31 & -c ) );
}
static __inline uint32_t ror( uint32_t x, unsigned c )
{
	return rol( x, -c );
}
 ...
 */
static __inline uint32_t des_S( uint32_t si6420, uint32_t si7531 )
{
	...
	*/
	uint64_t c6240 = ( ( si6420 | ( ( uint64_t )si6420 << 24 ) ) & 0x00FC00FC00FC00FC ) | 0xFF00FF00FF00FF00;
	uint64_t c7351 = ( ( si7531 | ( ( uint64_t )si7531 << 24 ) ) & 0x00FC00FC00FC00FC ) | 0xFF00FF00FF00FF00;
...
static __inline uint32_t des_benes_step( uint32_t v, unsigned D, uint32_t mask )
{
	uint32_t diff = ( v ^ ( v >> D ) ) & mask;
	return v ^ diff ^ ( diff << D );
}

static __inline uint32_t des_P( uint32_t v_orig )
{
	uint32_t v = v_orig;
...
static __inline uint32_t des_f( uint32_t R, uint32_t K7531, uint32_t K6420 )
{
 ...
 * be highly optimised.
 */
static __inline uint64_t bitsel( uint64_t input, const int8_t *bitnums, size_t size )
{
	uint64_t ret = 0;
	while ( size-- > 0 )
	{
...
static __inline LR des_load_lr( const void *vp )
{
...
static __inline void des_store_lr( void *vp, LR lr )
{
...
static __inline LR des_xor_lr( LR a, LR b )
{
	a.L ^= b.L;
	a.R ^= b.R;
	return a;
...
static __inline LR des_swap_lr( LR in )
{
	LR out;
	out.L = in.R;
	out.R = in.L;
...
static __inline void des_bitswap_IP_FP( uint32_t *L, uint32_t *R, unsigned D, uint32_t mask )
{
...
static __inline LR des_IP( LR lr )
{
...
static __inline LR des_FP( LR lr )
{
	lr.L = rol( lr.L, 1 );
	lr.R = rol( lr.R, 1 );
...
static __inline LR des_round( LR in, const des_keysched *sched, size_t round )
{
	LR out;
	out.L = in.R;
...
static __inline LR des_inner_cipher( LR lr, const des_keysched *sched, size_t start, size_t step )
{
...
static __inline LR des_full_cipher( LR lr, const des_keysched *sched, size_t start, size_t step )
{
	lr = des_IP( lr );
...
{
	struct des3_sdctr_ctx *ctx = container_of( ciph, struct des3_sdctr_ctx, ciph );
	smemclr( ctx, sizeof( *ctx ) );
	sfree( ctx );
}
...
static void des3_sdctr_setiv( ssh_cipher *ciph, const void *viv )
{
	...
	{
		ctx->counter[ i ] = GET_BIGNUMINT_MSB_FIRST( iv + 8 - BIGNUM_INT_BYTES - i * BIGNUM_INT_BYTES );
	}
}
...
static void des3_sdctr_encrypt_decrypt( ssh_cipher *ciph, void *vdata, int len )
{
	...
	for ( ; len > 0; len -= 8, data += 8 )
	{
		...
		for ( unsigned i = 0; i < SDCTR_WORDS; i++ )
		{
			PUT_BIGNUMINT_MSB_FIRST( iv_buf + 8 - BIGNUM_INT_BYTES - i * BIGNUM_INT_BYTES, ctx->counter[ i ] );
		}
