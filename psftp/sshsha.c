...
{
	const ssh_hashalg *real_alg = sha1_hw_available_cached() ? &ssh_sha1_hw : &ssh_sha1_sw;
...
static __inline void sha1_block_setup( sha1_block *blk )
{
	blk->used = 0;
	blk->len = 0;
}

static __inline bool sha1_block_write( sha1_block *blk, const void **vdata, size_t *len )
{
...
static __inline void sha1_block_pad( sha1_block *blk, BinarySink *bs )
{
...
static __inline uint32_t rol( uint32_t x, unsigned y )
{
...
static __inline uint32_t Par( uint32_t x, uint32_t y, uint32_t z )
{
	return ( x ^ y ^ z );
}

static __inline void sha1_sw_round( unsigned round_index, const uint32_t *schedule, uint32_t *a, uint32_t *b, uint32_t * /*c*/, uint32_t * /*d*/, uint32_t *e, uint32_t f, uint32_t constant )
{
...
*/
FUNC_ISA
static __inline void sha1_ni_block( __m128i *core, const uint8_t *p )
{
...
FUNC_ISA static ssh_hash *sha1_ni_new( const ssh_hashalg *alg )
{
	...
	s->core[ 0 ] = _mm_set_epi64x( 0x67452301efcdab89ULL, 0x98badcfe10325476ULL );
...
static ssh_hash *sha1_stub_copy( ssh_hash * /*hash*/ ) STUB_BODY_RET
...
const ssh_hashalg ssh_sha1_hw =
{
	...
	20, 64, HASHALG_NAMES_ANNOTATED( "SHA-1", "!NONEXISTENT ACCELERATED VERSION!" ),
};
