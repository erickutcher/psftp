...
static void sftp_pkt_BinarySink_write( BinarySink *bs, const void *data, size_t length )
{
	struct sftp_packet *pkt = BinarySink_DOWNCAST( bs, struct sftp_packet );

	assert( length <= 0xFFFFFFFFU - pkt->length );

	sgrowarrayn_nm( char *, pkt->data, pkt->maxlen, pkt->length, length );
...
void BinarySink_put_fxp_attrs( BinarySink *bs, struct fxp_attrs attrs )
{
	...
	{
		// We currently don't support sending any extended attributes.
	}*/
}
