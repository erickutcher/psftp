...
{
	return crt_modpow( input, key->private_exponent, key->modulus, key->p, key->q, key->iqmp );
}
...
static ssh_key *rsa2_new_priv( const ssh_keyalg *self, ptrlen pub, ptrlen priv )
{
	BinarySource src[ 1 ];
	ssh_key *sshk;
	RSAKey *rsa;
...
static ssh_key *rsa2_new_priv_openssh( const ssh_keyalg * /*self*/, BinarySource *src )
{
	RSAKey *rsa;
...
static __inline const ssh_hashalg *rsa2_hash_alg_for_flags( unsigned flags, const char **protocol_id_out )
{
...
static __inline ptrlen rsa_pkcs1_prefix_for_hash( const ssh_hashalg *halg )
{
	if ( halg == &ssh_sha1 )
	{
		/*
...
//	unreachable( "bad hash algorithm for RSA PKCS#1" );
	...
	return make_ptrlen( NULL, 0 );
}

static __inline size_t rsa_pkcs1_length_of_fixed_parts( const ssh_hashalg *halg )
{
	ptrlen asn1_prefix = rsa_pkcs1_prefix_for_hash( halg );
	return halg->hlen + asn1_prefix.len + 2;
}

static unsigned char *rsa_pkcs1_signature_string( size_t nbytes, const ssh_hashalg *halg, ptrlen data )
{
...
static void rsa2_sign( ssh_key *key, ptrlen data, unsigned flags, BinarySink *bs )
{
	RSAKey *rsa = container_of( key, RSAKey, sshk );
	unsigned char *bytes;
	size_t nbytes;
...
char *rsa2_invalid( ssh_key *key, unsigned flags )
{
	...
	if ( nbytes < rsa_pkcs1_length_of_fixed_parts( halg ) )
	{
		return dupprintf( "%" SIZEu "-bit RSA key is too short to generate %s signatures", bits, sign_alg_name );
	}
...
static void oaep_mask( const ssh_hashalg *h, void *seed, int seedlen, void *vdata, int datalen )
{
...
strbuf *ssh_rsakex_encrypt( prng *pr, RSAKey *rsa, const ssh_hashalg *h, ptrlen in )
{
	mp_int *b1, *b2;
	int k, i;
	char *p;
	...
	*/
	...
	int outlen = k;
	unsigned char *out = ( unsigned char * )strbuf_append( toret, outlen );
