 ...
 */
static void lz77_compress( struct LZ77Context *ctx, const unsigned char *data, int len );
...
static void lz77_advance( struct LZ77InternalContext *st, unsigned char c, int hash )
{
	int off;
...
static void lz77_compress( struct LZ77Context *ctx, const unsigned char *data, int len )
{
	...
	*/
	for ( i = 0; i < st->npending; i++ )
	{
		...
		{
			foo[ j ] = ( i + j < st->npending ? st->pending[ i + j ] : data[ i + j - st->npending ] );
		}
	...
	while ( len > 0 )
	{
		if ( len >= HASHCHARS )
		{
			...
			*/
			nmatch = 0;
			for ( off = st->hashtab[ hash ].first; off != INVALID; off = st->win[ off ].next )
			{
		...
		if ( nmatch > 0 )
		{
			...
			*/
			...
			while ( matchlen < len )
			{
				...
				{
					if ( CHARAT( matchlen ) == CHARAT( matchlen - matches[ i ].distance ) )
					{
						matches[ j++ ] = matches[ i ];
					}
				}
...
{
	struct ssh_zlib_compressor *comp = container_of( sc, struct ssh_zlib_compressor, sc );
...
void zlib_compress_block( ssh_compressor *sc, const unsigned char *block, int len, unsigned char **outblock, int *outlen, int minlen )
{
	struct ssh_zlib_compressor *comp = container_of( sc, struct ssh_zlib_compressor, sc );
	struct Outbuf *out = ( struct Outbuf * )comp->ectx.userdata;
	bool in_block;
 ...
 */
static struct zlib_table *zlib_mkonetab( int *codes, unsigned char *lengths, int nsyms, int pfx, int pfxbits, int bits )
{
	...
	for ( code = 0; code <= tab->mask; code++ )
	{
		...
		tab->table[ code ].nexttable = zlib_mkonetab( codes, lengths, nsyms, pfx | ( code << pfxbits ), pfxbits + bits, nbits );
	}
...
{
	struct zlib_decompress_ctx *dctx = container_of( dc, struct zlib_decompress_ctx, dc );
...
static int zlib_huflookup( unsigned long *bitsp, int *nbitsp, struct zlib_table *tab )
{
...
bool zlib_decompress_block( ssh_decompressor *dc, const unsigned char *block, int len, unsigned char **outblock, int *outlen )
{
	struct zlib_decompress_ctx *dctx = container_of( dc, struct zlib_decompress_ctx, dc );
	...
	while ( len > 0 || dctx->nbits > 0 )
	{
		...
		switch ( dctx->state )
		{
			case START:
			{
				...
				*/
				header = ( ( ( dctx->bits & 0xFF00 ) >> 8 ) | ( ( dctx->bits & 0x00FF ) << 8 ) );
				EATBITS( 16 );
			...
			case TREES_LENLEN:
			{
				...
				{
					dctx->lenlen[ lenlenmap[ dctx->lenptr++ ] ] = ( unsigned char )( dctx->bits & 7 );
					EATBITS( 3 );
				}
			...
			case TREES_LEN:
			{
				if ( dctx->lenptr >= dctx->hlit + dctx->hdist )
				{
					...
					dctx->currdisttable = zlib_mktable( dctx->lengths + dctx->hlit, dctx->hdist );
					zlib_freetable( &dctx->lenlentable );
				...
				else
				{
					...
					dctx->lenrep = ( code == 16 && dctx->lenptr > 0 ? dctx->lengths[ dctx->lenptr - 1 ] : 0 );
			...
			case GOTDISTSYM:
			{
				...
				while ( dctx->len-- )
				{
					zlib_emit_char( dctx, dctx->window[ ( dctx->winpos - dist ) & ( WINSIZE - 1 ) ] );
				}
			}
			break;
