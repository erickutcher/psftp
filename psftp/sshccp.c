...
static INLINE void chacha20_decrypt( struct chacha20 *ctx, unsigned char *blk, int len )
{
...
static void bigval_import_le( bigval *r, const void *vdata, int len )
{
	...
	for ( i = 0; i < len; i++ )
	{
		r->w[ i / BIGNUM_INT_BYTES ] |= ( BignumInt )data[ i ] << ( 8 * ( i % BIGNUM_INT_BYTES ) );
	}
}
...
static void poly1305_feed_chunk( struct poly1305 *ctx, const unsigned char *chunk, int len )
{
	bigval c;
	bigval_import_le( &c, chunk, len );
	c.w[ len / BIGNUM_INT_BYTES ] |= ( BignumInt )1 << ( 8 * ( len % BIGNUM_INT_BYTES ) );
	bigval_add( &c, &c, &ctx->h );
	bigval_mul_mod_p( &ctx->h, &c, &ctx->r );
}

static void poly1305_feed( struct poly1305 *ctx, const unsigned char *buf, int len )
{
...
static ssh2_mac *poly_ssh2_new( const ssh2_macalg *alg, ssh_cipher *cipher )
{
...
static void ccp_length_op( struct ccp_context *ctx, void * /*blk*/, int /*len*/, unsigned long seq )
{
	unsigned char iv[ 8 ];
	/*
...
static void ccp_encrypt_length( ssh_cipher *cipher, void *blk, int len, unsigned long seq )
{
	struct ccp_context *ctx = container_of( cipher, struct ccp_context, ciph );
...
static void ccp_decrypt_length( ssh_cipher *cipher, void *blk, int len, unsigned long seq )
{
	struct ccp_context *ctx = container_of( cipher, struct ccp_context, ciph );
	ccp_length_op( ctx, blk, len, seq );
