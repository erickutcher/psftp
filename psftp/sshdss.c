...
static ssh_key *dss_new_pub( const ssh_keyalg * /*self*/, ptrlen data )
{
	...
	if ( get_err( src ) || mp_eq_integer( dss->p, 0 ) || mp_eq_integer( dss->q, 0 ) )
	{
		/* Invalid key. */
...
static void dss_freekey( ssh_key *key )
{
	...
	if ( dss->p ) { mp_free( dss->p ); }
	if ( dss->q ) { mp_free( dss->q ); }
	if ( dss->g ) { mp_free( dss->g ); }
	if ( dss->y ) { mp_free( dss->y ); }
	if ( dss->x ) { mp_free( dss->x ); }
	sfree( dss );
}
...
static bool dss_verify( ssh_key *key, ptrlen sig, ptrlen data )
{
	...
	*/
	...
	{      /* bug not present; read admin fields */
		...
		if ( get_err( src ) || !ptrlen_eq_string( type, "ssh-dss" ) || sig.len != 40 )
		{
			return false;
		}
	}
...
static ssh_key *dss_new_priv_openssh( const ssh_keyalg * /*self*/, BinarySource *src )
{
	struct dss_key *dss;
	...
	if ( get_err( src ) || mp_eq_integer( dss->q, 0 ) || mp_eq_integer( dss->p, 0 ) )
	{
		/* Invalid key. */
		dss_freekey( &dss->sshk );
		return NULL;
...
static void dss_sign( ssh_key *key, ptrlen data, unsigned /*flags*/, BinarySink *bs )
{
	...
	mp_int *k = dss_gen_k( "DSA deterministic k generator", dss->q, dss->x, digest, sizeof( digest ) );
