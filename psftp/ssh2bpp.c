...
static PktOut *ssh2_bpp_new_pktout( Ssh *ssh, int type );
...
BinaryPacketProtocol *ssh2_bpp_new( struct DataTransferStats *stats, bool is_server )
{
...
void ssh2_bpp_new_outgoing_crypto( BinaryPacketProtocol *bpp, const ssh_cipheralg *cipher, const void *ckey, const void *iv, const ssh2_macalg *mac, bool etm_mode, const void *mac_key, const ssh_compression_alg *compression, bool delayed_compression )
{
	...
	if ( cipher )
	{
		...
		s->cbc_ignore_workaround = ( ( ssh_cipher_alg( s->out.cipher )->flags & SSH_CIPHER_IS_CBC ) && !( s->bpp.remote_bugs & BUG_CHOKES_ON_SSH2_IGNORE ) );
	}
	else
	{
		s->out.cipher = NULL;
...
void ssh2_bpp_new_incoming_crypto( BinaryPacketProtocol *bpp, const ssh_cipheralg *cipher, const void *ckey, const void *iv, const ssh2_macalg *mac, bool etm_mode, const void *mac_key, const ssh_compression_alg *compression, bool delayed_compression )
{
	struct ssh2_bpp_state *s;
	assert( bpp->vt == &ssh2_bpp_vtable );
	s = container_of( bpp, struct ssh2_bpp_state, bpp );
	...
	* input data in the queue. */
	queue_idempotent_callback( s->bpp.ssh, &s->bpp.ic_in_raw );
}
...
static void ssh2_bpp_handle_input( BinaryPacketProtocol *bpp )
{
	...
	while ( 1 )
	{
		...
		if ( s->in.cipher && ( ssh_cipher_alg( s->in.cipher )->flags & SSH_CIPHER_IS_CBC ) && s->in.mac && !s->in.etm_mode )
		{
			/*
			...
			*/
			...
			for ( ;; )
			{ /* Once around this loop per cipher block. */
				/* Read another cipher-block's worth, and tack it on to the end. */
				BPP_READ( s->buf + ( s->packetlen + s->maclen ), s->cipherblk );
				/* Decrypt one more block (a little further back in the stream). */
				ssh_cipher_decrypt( s->in.cipher, s->buf + s->packetlen, s->cipherblk );

				/* Feed that block to the MAC. */
				put_data( s->in.mac, s->buf + s->packetlen, s->cipherblk );
				s->packetlen += s->cipherblk;

				/* See if that gives us a valid packet. */
				if ( ssh2_mac_verresult( s->in.mac, s->buf + s->packetlen ) && ( ( s->len = toint( GET_32BIT_MSB_FIRST( s->buf ) ) ) == s->packetlen - 4 ) )
				{
					break;
				}
		...
		else if ( s->in.mac && s->in.etm_mode )
		{
			...
			*/
			...
			if ( s->in.cipher && ( ssh_cipher_alg( s->in.cipher )->flags & SSH_CIPHER_SEPARATE_LENGTH ) )
			{
				/* Keep the packet the same though, so the MAC passes */
				unsigned char len[ 4 ];
				memcpy( len, s->buf, 4 );
				ssh_cipher_decrypt_length( s->in.cipher, len, 4, s->in.sequence );
			...
			/*
			* _Completely_ silly lengths should be stomped on before they do us any more damage.
			*/
			if ( s->len < 0 || s->len > ( long )OUR_V2_PACKETLIMIT || s->len % s->cipherblk != 0 )
			{
				ssh_sw_abort( s->bpp.ssh );
				crStopV;
			}
			...
			* Check the MAC.
			*/
			if ( s->in.mac && !ssh2_mac_verify( s->in.mac, s->data, s->len + 4, s->in.sequence ) )
			{
				ssh_sw_abort(s->bpp.ssh );
				crStopV;
			}
			...
			if ( s->in.cipher )
			{
				ssh_cipher_decrypt( s->in.cipher, s->data + 4, s->packetlen - 4 );
			}
		}
		else
		{
			...
			/*
			* Acquire and decrypt the first block of the packet. This will contain the length and padding details.
			*/
			...
			* _Completely_ silly lengths should be stomped on before they do us any more damage.
			*/
			if ( s->len < 0 || s->len > ( long )OUR_V2_PACKETLIMIT || ( s->len + 4 ) % s->cipherblk != 0 )
			{
				ssh_sw_abort( s->bpp.ssh );
				crStopV;
			}
			...
			*/
			BPP_READ( s->data + s->cipherblk, s->packetlen + s->maclen - s->cipherblk );

			/* Decrypt everything _except_ the MAC. */
			if ( s->in.cipher )
			{
				ssh_cipher_decrypt( s->in.cipher, s->data + s->cipherblk, s->packetlen - s->cipherblk );
			}
		...
		*/
		{
			...
			int newlen;
			if ( s->in_decomp && ssh_decompressor_decompress( s->in_decomp, s->data + 5, s->length - 5, &newpayload, &newlen ) )
			{
		...
		*/
		...
		pq_push( s->bpp.ssh, &s->bpp.in_pq, s->pktin );

		{
			...
			if ( s->pending_compression && userauth_range( type ) )
			{
				...
				*/
				...
				queue_idempotent_callback( s->bpp.ssh, &s->bpp.ic_out_pq );
			}
		}
	}
...
eof:
	...
	*/
	crMaybeWaitUntilV( !pq_peek( s->bpp.ssh, &s->bpp.in_pq ) );
...
static PktOut *ssh2_bpp_new_pktout( Ssh * /*ssh*/, int pkt_type )
{
...
static void ssh2_bpp_format_packet_inner( struct ssh2_bpp_state *s, PktOut *pkt )
{
	...
	if ( s->out_comp )
	{
		...
		*/
		...
		ssh_compressor_compress( s->out_comp, pkt->data + 5, pkt->length - 5, &newpayload, &newlen, minlen );
		pkt->length = 5;
	...
	*/
	...
	padding += ( cipherblk - ( pkt->length - unencrypted_prefix + padding ) % cipherblk ) % cipherblk;
	assert( padding <= 255 );
	...
	random_read( s->bpp.ssh->pr, pkt->data + origlen, padding );
	pkt->data[ 4 ] = padding;
	PUT_32BIT_MSB_FIRST( pkt->data, origlen + padding - 4 );

	/* Encrypt length if the scheme requires it */
	if ( s->out.cipher && ( ssh_cipher_alg( s->out.cipher )->flags & SSH_CIPHER_SEPARATE_LENGTH ) )
	{
	...
	if ( s->out.mac && s->out.etm_mode )
	{
		...
		*/
		if ( s->out.cipher )
		{
			ssh_cipher_encrypt( s->out.cipher, pkt->data + 4, origlen + padding - 4 );
		}
		ssh2_mac_generate( s->out.mac, pkt->data, origlen + padding, s->out.sequence );
	}
	else
	{
		/*
		...
		*/
		...
		if ( length < pkt->minlen )
		{
			...
			*/
			...
			/*
			* And work backwards from that to the length of the contained string.
			*/
			if ( s->out.mac )
			{
				length -= ssh2_mac_alg( s->out.mac )->len;
			...
			ignore_pkt = ssh2_bpp_new_pktout( s->bpp.ssh, SSH2_MSG_IGNORE );
			...
			random_read( s->bpp.ssh->pr, ignore_pkt->data + origlen, length );
			ssh2_bpp_format_packet_inner( s, ignore_pkt );
			bufchain_add( s->bpp.ssh, s->bpp.out_raw, ignore_pkt->data, ignore_pkt->length );
			ssh_free_pktout( ignore_pkt );
		}
	}

	ssh2_bpp_format_packet_inner( s, pkt );
	bufchain_add( s->bpp.ssh, s->bpp.out_raw, pkt->data, pkt->length );
}
...
static void ssh2_bpp_handle_output( BinaryPacketProtocol *bpp )
{
	...
	*/
	n_userauth = 0;
	for ( pkt = pq_first( s->bpp.ssh, &s->bpp.out_pq ); pkt != NULL; pkt = pq_next( s->bpp.ssh, &s->bpp.out_pq, pkt ) )
	{
	...
	if ( s->cbc_ignore_workaround )
	{
		...
		* from out_raw).
		*/
		if ( bufchain_size( s->bpp.out_raw ) < ( ssh_cipher_alg( s->out.cipher )->blksize + ssh2_mac_alg( s->out.mac )->len ) )
		{
			/*
			...
			* IGNORE.
			*/
			pkt = ssh_bpp_new_pktout( s->bpp.ssh, &s->bpp, SSH2_MSG_IGNORE );
			put_stringz( pkt, "" );
			ssh2_bpp_format_packet( s, pkt );
		}
	}

	while ( ( pkt = pq_pop( s->bpp.ssh, &s->bpp.out_pq ) ) != NULL )
	{
		int type = pkt->type;
