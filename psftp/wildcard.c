 ...
 */
static int wc_match_fragment( const char **fragment, const char **target, const char *target_end )
{
	const char *f, *t;
 ...
 */
static int wc_match_inner( const char *wildcard, const char *target, size_t target_len )
{
...
{
	return wc_match_inner( wildcard, ( const char * )target.ptr, target.len );
}
...
bool wc_unescape( char *output, const char *wildcard )
{
	while ( *wildcard )
	{
		...
		else if ( *wildcard == '*' || *wildcard == '?' || *wildcard == '[' || *wildcard == ']' )
		{
