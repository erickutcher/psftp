 ...
 */
static __inline size_t size_t_min( size_t a, size_t b )
{
	return a < b ? a : b;
}
static __inline size_t size_t_max( size_t a, size_t b )
{
	return a > b ? a : b;
}
 ...
 */
static __inline BignumInt mp_word( mp_int *x, size_t i )
{
...
static uintmax_t shift_right_by_one_word( uintmax_t n )
{
	...
	return shift_too_big ? 0 : n >> ( shift_too_big ? 0 : BIGNUM_INT_BITS );
}
...
static mp_int *mp_make_sized( size_t nw )
{
	...
	x->nw = nw;
	x->w = ( BignumInt * )snew_plus_get_aux( x );
	mp_clear( x );
	return x;
}
...
{
	mp_int *x = mp_make_sized( ( sizeof( n ) + BIGNUM_INT_BYTES - 1 ) / BIGNUM_INT_BYTES );
 ...
 */
void mp_select_into( mp_int *dest, mp_int *src0, mp_int *src1, unsigned which )
{
...
static mp_int *mp_from_bytes_int( ptrlen bytes, size_t m, size_t c )
{
	...
	{
		n->w[ i / BIGNUM_INT_BYTES ] |= ( BignumInt )( ( ( const unsigned char * )bytes.ptr )[ m * i + c ] ) << ( 8 * ( i % BIGNUM_INT_BYTES ) );
	}
	return n;
}
...
mp_int *mp_from_hex_pl( ptrlen hex )
{
	...
	for ( size_t nibble = 0; nibble < hex.len; nibble++ )
	{
		...
		BignumInt lmask = ~-( ( BignumInt )( ( digit - 'a' ) | ( 'f' - digit ) ) >> ( BIGNUM_INT_BITS - 1 ) );
		BignumInt umask = ~-( ( BignumInt )( ( digit - 'A' ) | ( 'F' - digit ) ) >> ( BIGNUM_INT_BITS - 1 ) );
...
{
	return 0xFF & ( mp_word( x, byte / BIGNUM_INT_BYTES ) >> ( 8 * ( byte % BIGNUM_INT_BYTES ) ) );
}

unsigned mp_get_bit( mp_int *x, size_t bit )
{
	return 1 & ( mp_word( x, bit / BIGNUM_INT_BITS ) >> ( bit % BIGNUM_INT_BITS ) );
}
 ...
 * value to 1.
 */
static __inline unsigned normalise_to_1( BignumInt n )
{
	n = ( n >> 1 ) | ( n & 1 );            /* ensure top bit is clear */
	n = ( BignumInt )( -n ) >> ( BIGNUM_INT_BITS - 1 ); /* normalise to 0 or 1 */
	return n;
}
static __inline unsigned normalise_to_1_u64( uint64_t n )
{
	n = ( n >> 1 ) | ( n & 1 );            /* ensure top bit is clear */
 ...
 */
static __inline void mp_find_highest_nonzero_word_pair( mp_int *x, size_t shift_wanted, size_t *index, uint64_t *hi, uint64_t *lo )
{
	...
	for ( size_t curr_index = 0; curr_index < x->nw; curr_index++ )
	{
		...
		curr_lo = ( BIGNUM_INT_BITS < 64 ? ( curr_lo >> BIGNUM_INT_BITS ) : 0 ) | ( curr_hi << ( 64 - BIGNUM_INT_BITS ) );
		curr_hi = ( BIGNUM_INT_BITS < 64 ? ( curr_hi >> BIGNUM_INT_BITS ) : 0 ) | ( ( uint64_t )curr_word << shift_wanted );
...
size_t mp_get_nbits( mp_int *x )
{
	...
	*/
	...
	for ( size_t i = ( 1 << ( BIGNUM_INT_BITS_BITS - 1 ) ); i != 0; i >>= 1 )
	{
		...
		BignumInt indicator = ( BignumInt )( -shifted_word ) >> ( BIGNUM_INT_BITS - 1 );
...
/*
 * Routines for reading and writing the SSH-2 wire formats
...
mp_int *BinarySource_get_mp_ssh2( BinarySource *src )
{
	...
	else
	{
		...
		if ( ( bytes.len > 0 && ( ( p[ 0 ] & 0x80 ) || ( p[ 0 ] == 0 && ( bytes.len <= 1 || !( p[ 1 ] & 0x80 ) ) ) ) ) )
		{
			src->err = BSE_INVALID;
			return mp_from_integer(0);
		}
 ...
 * == 0.
 */
static BignumCarry mp_add_masked_into( BignumInt *w_out, size_t rw, mp_int *a, mp_int *b, BignumInt b_and, BignumInt b_xor, BignumCarry carry )
{
 ...
 */
static __inline BignumCarry mp_add_into_internal( mp_int *r, mp_int *a, mp_int *b )
{
 ...
 */
static BignumCarry mp_add_masked_integer_into( BignumInt *w_out, size_t rw, mp_int *a, uintmax_t b, BignumInt b_and, BignumInt b_xor, BignumCarry carry )
{
	for ( size_t i = 0; i < rw; i++ )
	{
...
{
	mp_add_masked_integer_into( r->w, r->nw, a, n, ~( BignumInt )0, ~( BignumInt )0, 1 );
}
 ...
 */
static void mp_add_integer_into_shifted_by_words( mp_int *r, mp_int *a, uintmax_t n, size_t word_index )
{
...
static void mp_mul_add_simple( mp_int *r, mp_int *a, mp_int *b )
{
	...
	for ( BignumInt *ap = a->w, *rp = r->w; ap < aend && rp < rend; ap++, rp++ )
	{
...
static __inline size_t mp_mul_scratchspace_unary( size_t n )
{
	/*
...
static void mp_mul_internal( mp_int *r, mp_int *a, mp_int *b, mp_int scratch )
{
	...
	* in the output bignum. They can't overlap. */
	...
	if ( r->nw < inlen * 2 )
	{
		...
		* and add each one in.
		*/
		mp_int s = mp_alloc_from_scratch( &scratch, size_t_min( botlen + toplen, r1.nw ) );
...
mp_int *mp_invert_mod_2to( mp_int *x, size_t p )
{
	...
	* can't be zero */
	...
	mp_int mul_scratch = mp_alloc_from_scratch( &scratch_per_iter, mul_scratchsize );
...
MontyContext *monty_new( mp_int *modulus )
{
	...
	{
		mc->powers_of_r_mod_m[ j ] = mp_modmul( mc->powers_of_r_mod_m[ 0 ], mc->powers_of_r_mod_m[ j - 1 ], mc->m );
	}
 ...
 */
static void mp_bezout_into( mp_int *a_coeff_out, mp_int *b_coeff_out, mp_int *a_in, mp_int *b_in )
{
	...
	* debug this code. */
	...
	mp_int *record = mp_make_sized( ( steps * 2 + BIGNUM_INT_BITS - 1 ) / BIGNUM_INT_BITS );
	...
	for ( size_t step = 0; step < steps; step++ )
	{
		...
		*/
		...
		unsigned swap = swap_if_one_even ^ ( ( swap_if_both_odd ^ swap_if_one_even ) & both_odd );
...
void mp_divmod_into( mp_int *n, mp_int *d, mp_int *q_out, mp_int *r_out )
{
	...
	*/
	...
	mp_find_highest_nonzero_word_pair( d, 64 - BIGNUM_INT_BITS, &hiword_index, &hibits, &lobits );
	...
	*/
	...
	size_t output_bit_index;
	{
		...
		size_t input_bit_index = ( hiword_index * BIGNUM_INT_BITS ) - ( 128 - BIGNUM_INT_BITS );
		...
		* of words. */
		...
		mp_add_integer_into_shifted_by_words( r_approx, r_approx, lobits, output_word_index );
		mp_add_integer_into_shifted_by_words( r_approx, r_approx, hibits, output_word_index + 64 / BIGNUM_INT_BITS );
	}
	...
	*/
	...
	mp_add_integer_into_shifted_by_words( two_R, two_R, ( BignumInt )1 << ( ( log2_R + 1 ) % BIGNUM_INT_BITS), ( log2_R + 1 ) / BIGNUM_INT_BITS );
	...
	*/
	...
	size_t scratchsize = size_t_max( mp_mul_scratchspace( dr->nw, r_approx->nw, d->nw ), mp_mul_scratchspace( product->nw, r_approx->nw, diff->nw ) );
	mp_int *scratch = mp_make_sized( scratchsize );
	mp_int product_shifted = mp_make_alias( product, log2_R / BIGNUM_INT_BITS, product->nw );
	...
	*/
	while ( good_bits < good_bits_needed )
	{
		...
		*/
		...
		mp_rshift_fixed_into( r_approx, &product_shifted, log2_R % BIGNUM_INT_BITS );
	...
	*/
	...
	mp_int quotient_alias = mp_make_alias( quotient_full, log2_R / BIGNUM_INT_BITS, quotient_full->nw );
	...
	*/
	...
	for ( size_t i = 0; i < sc->e; i++ )
	{
		...
		else
		{
			...
			monty_mul_into( sc->mc, &power_of_zk, &power_of_zk, &power_of_zk );
...
mp_int *mp_random_bits_fn( prng *pr, size_t bits, random_read_fn_t random_read )
{
	size_t bytes = ( bits + 7 ) / 8;
	uint8_t *randbuf = snewn( bytes, uint8_t );
	random_read( pr, randbuf, bytes );
	if ( bytes )
	{
...
mp_int *mp_random_in_range_fn( prng *pr, mp_int *lo, mp_int *hi, random_read_fn_t rf )
{
	...
	*/
	mp_int *unreduced = mp_random_bits_fn( pr, mp_max_bits( n_outcomes ) + 128, rf );
