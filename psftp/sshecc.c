...
static void initialise_common( struct ec_curve *curve, EllipticCurveType type, mp_int *p )
{
	curve->type = type;
	curve->p = mp_copy( p );
...
static void initialise_wcurve( struct ec_curve *curve, mp_int *p, mp_int *a, mp_int *b, mp_int *nonsquare, mp_int *G_x, mp_int *G_y, mp_int *G_order )
{
...
static void initialise_mcurve( struct ec_curve *curve, mp_int *p, mp_int *a, mp_int *b, mp_int *G_x, unsigned log2_cofactor )
{
...
static void initialise_ecurve( struct ec_curve *curve, mp_int *p, mp_int *d, mp_int *a, mp_int *nonsquare, mp_int *G_x, mp_int *G_y, mp_int *G_order )
{
...
WeierstrassPoint *ecdsa_public( mp_int *private_key, const ssh_keyalg *alg )
{
	...
	WeierstrassPoint *toret = ecc_weierstrass_multiply( curve->w.G, priv_reduced );
	mp_free( priv_reduced );
	return toret;
}

static mp_int *eddsa_exponent_from_hash( ptrlen hash, const struct ec_curve *curve )
{
	/*
...
EdwardsPoint *eddsa_public( mp_int *private_key, const ssh_keyalg *alg )
{
	...
	mp_int *exponent = eddsa_exponent_from_hash( make_ptrlen( hash, extra->hash->hlen ), curve );
...
#define put_mp_le_unsigned( bs, x )	BinarySink_put_mp_le_unsigned( BinarySink_UPCAST( bs ), x )

static WeierstrassPoint *ecdsa_decode( ptrlen encoded, const struct ec_curve *curve )
{
	assert( curve->type == EC_WEIERSTRASS );
	BinarySource src[ 1 ];
...
static WeierstrassPoint *BinarySource_get_wpoint( BinarySource *src, const struct ec_curve *curve )
{
...
#define get_wpoint( src, curve )	BinarySource_get_wpoint( BinarySource_UPCAST( src ), curve )

static void BinarySink_put_wpoint( BinarySink *bs, WeierstrassPoint *point, const struct ec_curve *curve, bool bare )
{
	strbuf *sb = NULL;
	BinarySink *bs_inner;
...
#define put_wpoint( bs, point, curve, bare )	BinarySink_put_wpoint( BinarySink_UPCAST( bs ), point, curve, bare )
...
static EdwardsPoint *eddsa_decode( ptrlen encoded, const struct ec_curve *curve )
{
	...
	* the bottom bit of x. Extract it, and zero that bit in y. */
	...
	EdwardsPoint *P = ecc_edwards_point_new_from_y( curve->e.ec, y, desired_x_parity );
	mp_free( y );
...
static EdwardsPoint *BinarySource_get_epoint( BinarySource *src, const struct ec_curve *curve )
{
	ptrlen str = get_string( src );
	if ( get_err( src ) )
	{
...
#define get_epoint( src, curve )	BinarySource_get_epoint( BinarySource_UPCAST( src ), curve )

static void BinarySink_put_epoint( BinarySink *bs, EdwardsPoint *point, const struct ec_curve *curve, bool bare )
{
	mp_int *x, *y;
	...
	*/
	...
	put_byte( bs, ( mp_get_byte( y, curve->fieldBytes - 1 ) & 0x7F ) | ( mp_get_bit( x, 0 ) << 7 ) );

	mp_free( x );
	mp_free( y );
}
#define put_epoint( bs, point, curve, bare )	BinarySink_put_epoint( BinarySink_UPCAST( bs ), point, curve, bare )
...
static char *ecc_cache_str_shared( const char *curve_name, mp_int *x, mp_int *y )
{
...
static ssh_key *eddsa_new_priv_openssh( const ssh_keyalg *alg, BinarySource *src )
{
	const struct ecsign_extra *extra = ( const struct ecsign_extra * )alg->extra;
	struct ec_curve *curve = extra->curve();
	assert( curve->type == EC_EDWARDS );
...
static ssh_key *ecdsa_new_priv_openssh( const ssh_keyalg *alg, BinarySource *src )
{
	const struct ecsign_extra *extra = ( const struct ecsign_extra * )alg->extra;
	struct ec_curve *curve = extra->curve();
	assert( curve->type == EC_WEIERSTRASS );
...
static mp_int *ecdsa_signing_exponent_from_data( const struct ec_curve *curve, const struct ecsign_extra *extra, ptrlen data )
{
...
{
	struct ecdsa_key *ek = container_of( key, struct ecdsa_key, sshk );
	const struct ecsign_extra *extra = ( const struct ecsign_extra * )ek->sshk.vt->extra;
...
static mp_int *eddsa_signing_exponent_from_data( struct eddsa_key *ek, const struct ecsign_extra *extra, ptrlen r_encoded, ptrlen data )
{
...
static void ecdsa_sign( ssh_key *key, ptrlen data, unsigned /*flags*/, BinarySink *bs )
{
	struct ecdsa_key *ek = container_of( key, struct ecdsa_key, sshk );
	const struct ecsign_extra *extra = ( const struct ecsign_extra * )ek->sshk.vt->extra;
	assert( ek->privateKey );
	...
	* k generation system we use for conventional DSA. */
	mp_int *k;
	{
		...
		k = dss_gen_k( "ECDSA deterministic k generator", ek->curve->w.G_order, ek->privateKey, digest, sizeof( digest ) );
	}
...
static void eddsa_sign( ssh_key *key, ptrlen data, unsigned /*flags*/, BinarySink *bs )
{
	struct eddsa_key *ek = container_of( key, struct eddsa_key, sshk );
	const struct ecsign_extra *extra = ( const struct ecsign_extra * )ek->sshk.vt->extra;
	assert( ek->privateKey );
	...
	*/
	mp_int *a = eddsa_exponent_from_hash( make_ptrlen( hash, ek->curve->fieldBytes ), ek->curve );
	...
	*/
	...
	put_data( h, hash + ek->curve->fieldBytes, extra->hash->hlen - ek->curve->fieldBytes );
	put_datapl( h, data );
	ssh_hash_final( h, hash );
	mp_int *log_r_unreduced = mp_from_bytes_le( make_ptrlen( hash, extra->hash->hlen ) );
	...
	* eddsa_verify does.
	*/
	mp_int *H = eddsa_signing_exponent_from_data( ek, extra, ptrlen_from_strbuf( r_enc ), data );
...
{
	struct ec_curve *( *curve )( void );
	void ( *setup )( prng *pr, ecdh_key *dh );
...
static void ssh_ecdhkex_w_setup( prng *pr, ecdh_key *dh )
{
	mp_int *one = mp_from_integer( 1 );
	dh->_private = mp_random_in_range( pr, one, dh->curve->w.G_order );
	mp_free( one );

	dh->w_public = ecc_weierstrass_multiply( dh->curve->w.G, dh->_private );
}

static void ssh_ecdhkex_m_setup( prng *pr, ecdh_key *dh )
{
	strbuf *bytes = strbuf_new_nm();
	random_read( pr, strbuf_append( bytes, dh->curve->fieldBytes ), dh->curve->fieldBytes );

	dh->_private = mp_from_bytes_le( ptrlen_from_strbuf( bytes ) );

	/* Ensure the private key has the highest valid bit set, and no
	* bits _above_ the highest valid one */
	mp_reduce_mod_2to( dh->_private, dh->curve->fieldBits );
	mp_set_bit( dh->_private, dh->curve->fieldBits - 1, 1 );

	/* Clear a curve-specific number of low bits */
	for ( unsigned bit = 0; bit < dh->curve->m.log2_cofactor; bit++ )
	{
		mp_set_bit( dh->_private, bit, 0 );
	}

	strbuf_free( bytes );

	dh->m_public = ecc_montgomery_multiply( dh->curve->m.G, dh->_private );
}
...
static mp_int *ssh_ecdhkex_w_getkey( ecdh_key *dh, ptrlen remoteKey )
{
	...
	WeierstrassPoint *p = ecc_weierstrass_multiply( remote_p, dh->_private );
...
static mp_int *ssh_ecdhkex_m_getkey( ecdh_key *dh, ptrlen remoteKey )
{
	...
	* will be reduced mod p. */
	...
	MontgomeryPoint *remote_p = ecc_montgomery_point_new( dh->curve->m.mc, remote_x );
	mp_free( remote_x );

	MontgomeryPoint *p = ecc_montgomery_multiply( remote_p, dh->_private );
	mp_int *x;
