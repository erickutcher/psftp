...
DECL_WINDOWS_FUNCTION( static, SECURITY_STATUS, AcquireCredentialsHandleA, ( SEC_CHAR *, SEC_CHAR *, ULONG, PVOID, PVOID, SEC_GET_KEY_FN, PVOID, PCredHandle, PTimeStamp ) );
DECL_WINDOWS_FUNCTION( static, SECURITY_STATUS, InitializeSecurityContextA, ( PCredHandle, PCtxtHandle, SEC_CHAR *, ULONG, ULONG, ULONG, PSecBufferDesc, ULONG, PCtxtHandle, PSecBufferDesc, PULONG, PTimeStamp ) );
DECL_WINDOWS_FUNCTION( static, SECURITY_STATUS, FreeContextBuffer, ( PVOID ) );
DECL_WINDOWS_FUNCTION( static, SECURITY_STATUS, FreeCredentialsHandle, ( PCredHandle ) );
DECL_WINDOWS_FUNCTION( static, SECURITY_STATUS, DeleteSecurityContext, ( PCtxtHandle ) );
DECL_WINDOWS_FUNCTION( static, SECURITY_STATUS, QueryContextAttributesA, ( PCtxtHandle, ULONG, PVOID ) );
DECL_WINDOWS_FUNCTION( static, SECURITY_STATUS, MakeSignature, ( PCtxtHandle, ULONG, PSecBufferDesc, ULONG ) );
DECL_WINDOWS_FUNCTION( static, SECURITY_STATUS, VerifySignature, ( PCtxtHandle, PSecBufferDesc, ULONG, PULONG ) );
...
struct ssh_gss_liblist *g_gss_libs = NULL;
...
void ssh_gss_setup()
{
	if ( !g_gss_loaded )
	{
		EnterCriticalSection( &gss_library_cs );

		if ( !g_gss_loaded )
		{
			HMODULE module;
			HKEY regkey;
			g_gss_libs = snew( struct ssh_gss_liblist );
			...
			g_gss_libs->libraries = snewn( ngsslibs, struct ssh_gss_library );
			g_gss_libs->nlibraries = 0;
			...
			if ( RegOpenKey( HKEY_LOCAL_MACHINE, "SOFTWARE\\MIT\\Kerberos", &regkey ) == ERROR_SUCCESS )
			{
				...
				if ( ret == ERROR_SUCCESS && type == REG_SZ )
				{
					...
					ret = RegQueryValueEx( regkey, "InstallDir", NULL, &type, ( LPBYTE )buffer, &size );
					if ( ret == ERROR_SUCCESS && type == REG_SZ )
					{
						strcat ( buffer, "\\bin" );
						if ( p_AddDllDirectory )
						{
							...
							* it loads its own DLLs further down the road */
							wchar_t *dllPath = dup_mb_to_wc( DEFAULT_CODEPAGE, 0, buffer );
			...
			if ( module )
			{
				struct ssh_gss_library *lib = &g_gss_libs->libraries[ g_gss_libs->nlibraries++ ];

				lib->id = 0;
				lib->handle = ( void * )module;

				#define BIND_GSS_FN( name )	lib->u.gssapi.name = ( t_gss_##name )GetProcAddress( module, "gss_" #name )
		...
		LeaveCriticalSection( &gss_library_cs );
	}
}
...
static Ssh_gss_stat ssh_sspi_indicate_mech( struct ssh_gss_library * /*lib*/, Ssh_gss_buf *mech )
{
	*mech = gss_mech_krb5;
	return SSH_GSS_OK;
}
...
static Ssh_gss_stat ssh_sspi_import_name( struct ssh_gss_library * /*lib*/, char *host, Ssh_gss_name *srv_name )
{
	char *pStr;
...
static Ssh_gss_stat ssh_sspi_acquire_cred( struct ssh_gss_library * /*lib*/, Ssh_gss_ctx *ctx, time_t *expiry )
{
	...
	/* Specifying no principal name here means use the credentials of the current logged-in user */

	winctx->maj_stat = p_AcquireCredentialsHandleA( NULL, "Kerberos", SECPKG_CRED_OUTBOUND, NULL, NULL, NULL, NULL, &winctx->cred_handle, NULL );
...
static void localexp_to_exp_lifetime( TimeStamp *localexp, time_t *expiry, unsigned long *lifetime )
{
	FILETIME nowUTC;
	FILETIME expUTC;
	time_t now;
											   ...
											   unsigned long *lifetime )
{
	...
	unsigned long flags = ISC_REQ_MUTUAL_AUTH | ISC_REQ_REPLAY_DETECT | ISC_REQ_CONFIDENTIALITY | ISC_REQ_ALLOCATE_MEMORY;
...
static Ssh_gss_stat ssh_sspi_free_tok( struct ssh_gss_library * /*lib*/, Ssh_gss_buf *send_tok )
{
	/* check input */
	if ( send_tok == NULL )
	{
...
static Ssh_gss_stat ssh_sspi_release_cred( struct ssh_gss_library * /*lib*/, Ssh_gss_ctx *ctx )
{
	winSsh_gss_ctx *winctx= ( winSsh_gss_ctx * )*ctx;
...
static Ssh_gss_stat ssh_sspi_release_name( struct ssh_gss_library * /*lib*/, Ssh_gss_name *srv_name )
{
...
static Ssh_gss_stat ssh_sspi_display_status( struct ssh_gss_library * /*lib*/, Ssh_gss_ctx ctx, Ssh_gss_buf *buf )
{
	winSsh_gss_ctx *winctx = ( winSsh_gss_ctx * )ctx;
	const char *msg;
	...
	switch ( winctx->maj_stat )
	{
		...
		case SEC_E_INVALID_HANDLE: { msg = "The handle passed to the function is invalid."; } break;
		case SEC_E_TARGET_UNKNOWN: { msg = "The target was not recognized."; } break;
		case SEC_E_LOGON_DENIED: { msg = "The logon failed."; } break;
		case SEC_E_INTERNAL_ERROR: { msg = "The Local Security Authority cannot be contacted."; } break;
		case SEC_E_NO_CREDENTIALS: { msg = "No credentials are available in the security package."; } break;
		...
		break;
		case SEC_E_INSUFFICIENT_MEMORY: { msg = "One or more of the SecBufferDesc structures passed as an OUT parameter has a buffer that is too small."; } break;
		...
		break;
		default: { msg = "Internal SSPI error"; } break;
	}
...
static Ssh_gss_stat ssh_sspi_get_mic( struct ssh_gss_library * /*lib*/, Ssh_gss_ctx ctx, Ssh_gss_buf *buf, Ssh_gss_buf *hash )
{
	winSsh_gss_ctx *winctx = ( winSsh_gss_ctx * )ctx;
	...
	winctx->maj_stat = p_QueryContextAttributesA( &winctx->context, SECPKG_ATTR_SIZES, &ContextSizes );

	if ( winctx->maj_stat != SEC_E_OK || ContextSizes.cbMaxSignature == 0 )
	{
	...
	winctx->maj_stat = p_MakeSignature( &winctx->context, 0, &InputBufferDescriptor, 0 );
...
static Ssh_gss_stat ssh_sspi_verify_mic( struct ssh_gss_library * /*lib*/, Ssh_gss_ctx ctx, Ssh_gss_buf *buf, Ssh_gss_buf *mic )
{
	winSsh_gss_ctx *winctx = ( winSsh_gss_ctx * )ctx;
	SecBufferDesc InputBufferDescriptor;
	SecBuffer InputSecurityToken[ 2 ];
	...
	winctx->maj_stat = p_VerifySignature( &winctx->context, &InputBufferDescriptor, 0, &qop );
	return ( Ssh_gss_stat )winctx->maj_stat;
}

static Ssh_gss_stat ssh_sspi_free_mic( struct ssh_gss_library * /*lib*/, Ssh_gss_buf *hash )
{
	sfree( hash->value );
	return SSH_GSS_OK;
}
