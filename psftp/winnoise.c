...
DECL_WINDOWS_FUNCTION( static, BOOL, CryptAcquireContextA, ( HCRYPTPROV *, LPCTSTR, LPCTSTR, DWORD, DWORD ) );
DECL_WINDOWS_FUNCTION( static, BOOL, CryptGenRandom, ( HCRYPTPROV, DWORD, BYTE * ) );
DECL_WINDOWS_FUNCTION( static, BOOL, CryptReleaseContext, ( HCRYPTPROV, DWORD ) );
...
bool win_read_random( void *buf, unsigned wanted )
{
	...
	if ( wincrypt_module && p_CryptAcquireContextA && p_CryptGenRandom && p_CryptReleaseContext && p_CryptAcquireContextA( &crypt_provider, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT ) )
	{
		toret = ( p_CryptGenRandom( crypt_provider, wanted, ( BYTE * )buf ) == FALSE ? false : true );
...
void noise_get_heavy( prng *pr, void ( *func )( prng *, void *, int ) )
{
	...
	if ( srch != INVALID_HANDLE_VALUE )
	{
		do
		{
			func( pr, &finddata, sizeof( finddata ) );
		}
	...
	func( pr, &pid, sizeof( pid ) );

	if ( win_read_random( buf, sizeof( buf ) ) )
	{
		func( pr, buf, sizeof( buf ) );
...
void noise_regular( prng *pr )
{
	...
	random_add_noise( pr, NOISE_SOURCE_FGWINDOW, &w, sizeof( w ) );
	w = GetCapture();
	random_add_noise( pr, NOISE_SOURCE_CAPTURE, &w, sizeof( w ) );
	w = GetClipboardOwner();
	random_add_noise( pr, NOISE_SOURCE_CLIPBOARD, &w, sizeof( w ) );
	z = GetQueueStatus( QS_ALLEVENTS );
	random_add_noise( pr, NOISE_SOURCE_QUEUE, &z, sizeof( z ) );

	GetCursorPos( &pt );
	random_add_noise( pr, NOISE_SOURCE_CURSORPOS, &pt, sizeof( pt ) );

	GlobalMemoryStatus( &memstat );
	random_add_noise( pr, NOISE_SOURCE_MEMINFO, &memstat, sizeof( memstat ) );

	GetThreadTimes( GetCurrentThread(), times, times + 1, times + 2, times + 3 );
	random_add_noise( pr, NOISE_SOURCE_THREADTIME, &times, sizeof( times ) );
	GetProcessTimes( GetCurrentProcess(), times, times + 1, times + 2, times + 3 );
	random_add_noise( pr, NOISE_SOURCE_PROCTIME, &times, sizeof( times ) );
}
...
void noise_ultralight( prng *pr, NoiseSourceId id, unsigned long data )
{
	...
	random_add_noise( pr, id, &data, sizeof( DWORD ) );

	wintime = GetTickCount();
	random_add_noise( pr, NOISE_SOURCE_TIME, &wintime, sizeof( DWORD ) );

	if ( QueryPerformanceCounter( &perftime ) )
	{
		random_add_noise( pr, NOISE_SOURCE_PERFCOUNT, &perftime, sizeof( perftime ) );
	}
}
