...
static void ssh_comp_none_block( ssh_compressor * /*handle*/, const unsigned char * /*block*/, int /*len*/, unsigned char ** /*outblock*/, int * /*outlen*/, int /*minlen*/ )
{
}
static bool ssh_decomp_none_block( ssh_decompressor * /*handle*/, const unsigned char * /*block*/, int /*len*/, unsigned char ** /*outblock*/, int * /*outlen*/ )
{
	return false;
}
...
static bool ssh2_transport_get_specials( PacketProtocolLayer *ppl, add_special_fn_t add_special, void *ctx );
static void ssh2_transport_special_cmd( PacketProtocolLayer *ppl, SessionSpecialCode code, int arg );
...
static void ssh2_transport_reconfigure( PacketProtocolLayer *ppl );
...
static void ssh2_transport_higher_layer_packet_callback( Ssh *ssh, void *context );
...
#ifndef NO_GSSAPI
static void ssh2_transport_gss_update( struct ssh2_transport_state *s, bool definitely_rekeying );
#endif

static bool ssh2_transport_timer_update( struct ssh2_transport_state *s, unsigned long rekey_time );
static int ssh2_transport_confirm_weak_crypto_primitive( struct ssh2_transport_state *s, const char *type, const char *name, const void *alg );
	...
	PacketProtocolLayer *higher_layer )
{
	...
	s->max_data_size = g_rekey_data_limit;
...
{
	struct ssh2_transport_state *s = container_of( ppl, struct ssh2_transport_state, ppl );
	...
	*/
	if ( s->ppl.out_pq )
	{
		pq_concatenate( s->ppl.ssh, s->ppl.out_pq, s->ppl.out_pq, &s->pq_out_higher );
	}

	ssh_ppl_free( s->higher_layer );

	pq_in_clear( s->ppl.ssh, &s->pq_in_higher );
	pq_out_clear( s->ppl.ssh, &s->pq_out_higher );
	...
	expire_timer_context( &s->ppl.ssh->timer_transport_rekey );
	sfree( s );
}
 ...
 */
static void ssh2_mkkey( struct ssh2_transport_state *s, strbuf *out, mp_int *K, unsigned char *H, char chr, int keylen )
{
 ...
 */
static struct kexinit_algorithm *ssh2_kexinit_addalg( struct kexinit_algorithm *list, const char *name )
{
	int i;
...
bool ssh2_common_filter_queue( PacketProtocolLayer *ppl )
{
	...
	while ( ( pktin = pq_peek( ppl->ssh, ppl->in_pq ) ) != NULL )
	{
		switch ( pktin->type )
		{
...
static bool ssh2_transport_filter_queue( struct ssh2_transport_state *s )
{
	...
	while ( 1 )
	{
		...
		/* Pass on packets to the next layer if they're outside the range reserved for the transport protocol. */
		if ( pktin->type >= 50 )
		{
			...
			* the first NEWKEYS. */
			...
			pq_push( s->ppl.ssh, &s->pq_in_higher, pktin );
		}
		else
		{
			/* Anything else is a transport-layer packet that the main process_queue coroutine should handle. */
			return false;
		}
	}
}
	...
	bool first_time, bool can_gssapi_keyex, bool transient_hostkey_mode )
{
	...
	*/
	...
	for ( i = 0; i < KEX_MAX; i++ )
	{
		switch ( ssh->ssh_kexlist[ i ] )
		{
			case KEX_DHGEX:		{ preferred_kex[ n_preferred_kex++ ] = &ssh_diffiehellman_gex; } break;
			case KEX_DHGROUP14:	{ preferred_kex[ n_preferred_kex++ ] = &ssh_diffiehellman_group14; } break;
			case KEX_DHGROUP1:	{ preferred_kex[ n_preferred_kex++ ] = &ssh_diffiehellman_group1; } break;
			case KEX_RSA:		{ preferred_kex[ n_preferred_kex++ ] = &ssh_rsa_kex; } break;
			case KEX_ECDH:		{ preferred_kex[ n_preferred_kex++ ] = &ssh_ecdh_kex; } break;
			case KEX_WARN:
			{
				/* Flag for later. Don't bother if it's the last in the list. */
				if ( i < KEX_MAX - 1 )
				{
	...
	*/
	...
	for ( i = 0; i < HK_MAX; i++ )
	{
		int id = ssh->ssh_hklist[ i ];
		/* As above, don't bother with HK_WARN if it's last in the list */
		if ( id == HK_MAX )
		{
			break;
		}
	...
	*/
	...
	{
		switch ( ssh->ssh_cipherlist[ i ] )
		{
			case CIPHER_BLOWFISH:	{ preferred_ciphers[ n_preferred_ciphers++ ] = &ssh2_blowfish; } break;
			case CIPHER_DES:		{ preferred_ciphers[ n_preferred_ciphers++ ] = &ssh2_des; } break;
			case CIPHER_3DES:		{ preferred_ciphers[ n_preferred_ciphers++ ] = &ssh2_3des; } break;
			case CIPHER_AES:		{ preferred_ciphers[ n_preferred_ciphers++ ] = &ssh2_aes; } break;
			case CIPHER_ARCFOUR:	{ preferred_ciphers[ n_preferred_ciphers++ ] = &ssh2_arcfour; } break;
			case CIPHER_CHACHA20:	{ preferred_ciphers[ n_preferred_ciphers++ ] = &ssh2_ccp; } break;
			case CIPHER_WARN:
			{
				/* Flag for later. Don't bother if it's the last in the list. */
				if ( i < CIPHER_MAX - 1 )
	...
	*/
	...
	for ( i = 0; i < n_preferred_kex; i++ )
	{
		...
		else
		{
			...
			{
				alg = ssh2_kexinit_addalg( kexlists[ KEXLIST_KEX ], k->list[ j ]->name );
	...
	{
		/*
		* In server mode, we just list the algorithms that match the host keys we actually have.
		*/
		for ( i = 0; i < our_nhostkeys; i++ )
		{
			alg = ssh2_kexinit_addalg( kexlists[ KEXLIST_HOSTKEY ], ssh_key_alg( our_hostkeys[ i ] )->ssh_id );
		...
		*/
		...
		for ( i = 0; i < n_preferred_hk; i++ )
		{
			...
			for ( j = 0; j < lenof( ssh2_hostkey_algs ); j++ )
			{
				...
				if ( g_CONF_ssh_prefer_known_hostkeys && have_ssh_host_key( ssh, ssh2_hostkey_algs[ j ].alg->ssh_id ) )
				{
					alg = ssh2_kexinit_addalg( kexlists[ KEXLIST_HOSTKEY ], ssh2_hostkey_algs[ j ].alg->ssh_id );
		...
		*/
		...
		for ( i = 0; i < n_preferred_hk; i++ )
		{
			...
			for ( j = 0; j < lenof( ssh2_hostkey_algs ); j++ )
			{
				...
				if ( ssh_transient_hostkey_cache_has( thc, ssh2_hostkey_algs[ j ].alg ) )
				{
					alg = ssh2_kexinit_addalg( kexlists[ KEXLIST_HOSTKEY ], ssh2_hostkey_algs[ j ].alg->ssh_id );
					alg->u.hk.hostkey = ssh2_hostkey_algs[ j ].alg;
					alg->u.hk.warn = warn;
	...
	for ( k = KEXLIST_CSCIPHER; k <= KEXLIST_SCCIPHER; k++ )
	{
		...
		for ( i = 0; i < n_preferred_ciphers; i++ )
		{
			...
			else
			{
				...
				{
					alg = ssh2_kexinit_addalg( kexlists[ k ], c->list[ j ]->ssh2_id );
	...
	*/
	...
	for ( j = KEXLIST_CSMAC; j <= KEXLIST_SCMAC; j++ )
	{
		...
		for ( i = 0; i < nmacs; i++ )
		{
			/* For each MAC, there may also be an ETM version, which we list second. */
	...
	/* List client->server compression algorithms, then server->client compression algorithms. (We use the same set twice.) */
	for ( j = KEXLIST_CSCOMP; j <= KEXLIST_SCCOMP; j++ )
	{
		...
		{
			alg = ssh2_kexinit_addalg( kexlists[ j ], preferred_comp->delayed_name );
			alg->u.comp.comp = preferred_comp;
	...
	/*
	* Finally, format the lists into text and write them into the outgoing KEXINIT packet.
	*/
	for ( i = 0; i < NKEXLIST; i++ )
	{
	...
	int *n_server_hostkeys, int server_hostkeys[ MAXKEXLIST ] )
{
	...
	* kexinit_algorithm structure. */
	for ( i = 0; i < NKEXLIST; i++ )
	{
		...
		for ( cfirst = true, clist = clists[ i ]; get_commasep_word( &clist, &cword ); cfirst = false )
		{
			for ( sfirst = true, slist = slists[ i ]; get_commasep_word( &slist, &sword ); sfirst = false )
			{
	...
	found_match:
		...
		for ( j = 0; j < MAXKEXLIST; j++ )
		{
			if ( kexlists[ i ][ j ].name && ptrlen_eq_string( found, kexlists[ i ][ j ].name ) )
			{
	...
	*/
	for ( i = 0; i < NKEXLIST; i++ )
	{
		...
		*/
		if ( i == KEXLIST_CSMAC && cs->cipher && cs->cipher->required_mac )
		{
			cs->mac = cs->cipher->required_mac;
			cs->etm_mode = !!( cs->mac->etm_name );
			continue;
		}
		if ( i == KEXLIST_SCMAC && sc->cipher && sc->cipher->required_mac )
		{
		...
		switch ( i )
		{
			...
			case KEXLIST_HOSTKEY:
			{
				...
				* selected.
				*/
				if ( alg->u.hk.hostkey == NULL && ( *kex_alg )->main_type != KEXTYPE_GSS )
				{
					continue;
				}
...
static void ssh2_transport_process_queue( PacketProtocolLayer *ppl )
{
	...
	// ssh->base_layer == s->ppl and ppl.
	// If it's NULL, then we freed it in the coroutine.
	Ssh *ssh = s->ppl.ssh;
...
begin_key_exchange:
	...
	else if ( g_CONF_try_gssapi_kex != 0 )
	{
		/*
		...
		*/
		...
		s->gss_delegate = g_CONF_gssapifwd;
	}
	else
	{
		s->can_gssapi_keyex = false;
...
//	s->ppl.bpp->pls->kctx = SSH2_PKTCTX_NOKEX;
	...
	/*
	* Construct our KEXINIT packet, in a strbuf so we can refer to it later.
	*/
	strbuf_clear( s->client_kexinit );
	put_byte( s->outgoing_kexinit, SSH2_MSG_KEXINIT );
	random_read( s->ppl.ssh->pr, strbuf_append( s->outgoing_kexinit, 16 ), 16 );
	...
	* Send our KEXINIT.
	*/
	pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, SSH2_MSG_KEXINIT );
	put_data( pktout, s->outgoing_kexinit->u + 1, s->outgoing_kexinit->len - 1 ); /* omit initial packet type byte */
	...
	*/
	...
	/*
	* Work through the two KEXINIT packets in parallel to find the selected algorithm identifiers.
	*/
	{
		...
		*/
		...
		for ( i = 0; i < nhk; i++ )
		{
			...
			if ( ssh2_hostkey_algs[ j ].alg != s->hostkey_alg && !have_ssh_host_key( s->ppl.ssh, ssh2_hostkey_algs[ j ].alg->ssh_id ) )
			{
	...
	*/
	...
	while ( 1 )
	{
		...
		EnterCriticalSection( &ssh2kex_cs );

		// This function is not thread safe.
		// It fails somewhere...somehow if it's not in a critical section.
		ssh2kex_coroutine( s, &aborted );

		LeaveCriticalSection( &ssh2kex_cs );

		if ( ssh->base_layer == NULL || aborted )
		{
	...
	*/
	pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, SSH2_MSG_NEWKEYS );
	pq_push( s->ppl.ssh, s->ppl.out_pq, pktout );
	...
	*/
	{
		...
		if ( s->out.cipher )
		{
			ssh2_mkkey( s, cipher_iv, s->K, s->exchange_hash, 'A' + s->out.mkkey_adjust, s->out.cipher->blksize );
			ssh2_mkkey( s, cipher_key, s->K, s->exchange_hash, 'C' + s->out.mkkey_adjust, s->out.cipher->padded_keybytes );
		}
		if ( s->out.mac )
		{
			ssh2_mkkey( s, mac_key, s->K, s->exchange_hash, 'E' + s->out.mkkey_adjust, s->out.mac->keylen );
		}
	...
	*/
	...
	/*
	* We've seen incoming NEWKEYS, so create and initialise incoming session keys.
	*/
	{
		strbuf *cipher_key = strbuf_new_nm();
		strbuf *cipher_iv = strbuf_new_nm();
		...
		if ( s->in.cipher )
		{
			ssh2_mkkey( s, cipher_iv, s->K, s->exchange_hash, 'A' + s->in.mkkey_adjust, s->in.cipher->blksize );
			ssh2_mkkey( s, cipher_key, s->K, s->exchange_hash, 'C' + s->in.mkkey_adjust, s->in.cipher->padded_keybytes );
		}
		if ( s->in.mac )
		{
			ssh2_mkkey( s, mac_key, s->K, s->exchange_hash, 'E' + s->in.mkkey_adjust, s->in.mac->keylen );
		}
	...
	*/
	if ( !s->higher_layer_ok )
	{
		if ( !s->hostkeys )
		{
			...
			pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, SSH2_MSG_SERVICE_REQUEST );
		...
		else
		{
			...
			pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, SSH2_MSG_SERVICE_ACCEPT );
			put_stringz( pktout, s->higher_layer->vt->name );
			pq_push( s->ppl.ssh, s->ppl.out_pq, pktout );
		}

		s->higher_layer_ok = true;
		queue_idempotent_callback( s->ppl.ssh, &s->higher_layer->ic_process_queue );
	}
...
static void ssh2_transport_timer( void *ctx, unsigned long now )
{
	...
	mins = sanitise_rekey_time( g_rekey_time, 60 );
	if ( mins == 0 )
	{
		return;
	}
	...
	{
		s->next_rekey = GetTickCount() + ticks;

		s->rekey_reason = "timeout";
		s->rekey_class = RK_NORMAL;
		queue_idempotent_callback( s->ppl.ssh, &s->ppl.ic_process_queue );
		return;
	}

#ifndef NO_GSSAPI
	/*
	* Rekey now if we have a new cred or context expires this cycle, but not if this is unsafe.
	*/
	if ( g_gss_rekey_time )
	{
...
static bool ssh2_transport_timer_update( struct ssh2_transport_state *s, unsigned long rekey_time )
{
	...
	if ( s->gss_kex_used )
	{
		...
		*/
		...
		gssmins = sanitise_rekey_time( g_gss_rekey_time, GSS_DEF_REKEY_MINS );
		if ( gssmins > 0 )
		{
			if ( gssmins < mins )
			{
			...
			if ( ( s->gss_status & GSS_KEX_CAPABLE ) != 0 )
			{
				...
				*/
				if ( ( s->gss_status & GSS_CTXT_EXPIRES ) == 0 && s->gss_ctxt_lifetime - mins * 60 < 2 * MIN_CTXT_LIFETIME )
				{
	...
	unsigned long next_rekey = GetTickCount() + ticks;

	/* Schedule the next timer */
	s->ppl.ssh->timer_transport_rekey.now = next_rekey;
	s->ppl.ssh->timer_transport_rekey.ctx = s;
	s->ppl.ssh->timer_transport_rekey.fn = ssh2_transport_timer;

	s->next_rekey = next_rekey;
...
static void ssh2_transport_gss_update( struct ssh2_transport_state *s, bool /*definitely_rekeying*/ )
{
	...
	/*
	* Nothing to do if no GSSAPI libraries are configured or GSSAPI auth is not enabled.
	*/
	if ( s->shgss == NULL || s->shgss->lib == NULL || ( g_CONF_try_gssapi_auth == 0 && g_CONF_try_gssapi_kex == 0 ) )
	{
		return;
	}
	...
	{
		gss_stat = s->shgss->lib->import_name( s->shgss->lib, s->fullhostname, &s->shgss->srv_name );
	...
	/*
	* Do we (still) have credentials? Capture the credential expiration when available
	*/
	gss_stat = s->shgss->lib->acquire_cred( s->shgss->lib, &gss_ctx, &gss_cred_expiry );
	if ( gss_stat != SSH_GSS_OK )
	{
		return;
	}
	...
	/*
	* When acquire_cred yields no useful expiration, get a proxy for the cred expiration from the context expiration.
	*/
	...
	if ( gss_stat != SSH_GSS_OK && gss_stat != SSH_GSS_S_CONTINUE_NEEDED )
	{
		/*
	...
	/*
	* When rekeying to cascade, avoding doing this too close to the context expiration time, since the key exchange might fail.
	*/
	...
	*/
	...
	if ( s->gss_cred_expiry != GSS_NO_EXPIRATION && difftime( gss_cred_expiry, s->gss_cred_expiry ) > 0 )
	{
		s->gss_status |= GSS_CRED_UPDATED;
	}

	mins = sanitise_rekey_time( g_gss_rekey_time, GSS_DEF_REKEY_MINS );
	...
	*/
	if ( !( s->ppl.remote_bugs & BUG_SSH2_REKEY ) )
	{
		...
		if ( s->n_uncert_hostkeys )
		{
			...
			{
				const ssh_keyalg *alg = ssh2_hostkey_algs[ s->uncert_hostkeys[ i ] ].alg;
...
static void ssh2_transport_reconfigure( PacketProtocolLayer *ppl )
{
	...
	rekey_time = sanitise_rekey_time( g_rekey_time, 60 );
	...
	s->max_data_size = g_rekey_data_limit;
	if ( old_max_data_size != s->max_data_size && s->max_data_size != 0 )
	{
	...
	ssh_ppl_reconfigure( s->higher_layer );
}
...
static size_t ssh2_transport_queued_data_size( PacketProtocolLayer *ppl )
{
	...
	return ( ssh_ppl_default_queued_data_size( ppl ) + ssh_ppl_queued_data_size( s->higher_layer ) );
}
