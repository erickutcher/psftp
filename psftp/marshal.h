...
#define BinarySink_INIT( obj, writefn )	( ( obj )->binarysink_->write = ( writefn ), ( obj )->binarysink_->binarysink_ = ( obj )->binarysink_ )
 ...
 */
#define BinarySink_DOWNCAST( object, type )	TYPECHECK( ( object ) == ( ( type * )0 )->binarysink_, ( ( type * )( ( ( char * )( object ) ) - offsetof( type, binarysink_ ) ) ) )
#define BinarySink_UPCAST( object )			TYPECHECK( ( object )->binarysink_ == ( BinarySink * )0, ( object )->binarysink_ )
 ...
 * object.
 */
#define BinarySink_COPIED( obj )	( ( obj )->binarysink_->binarysink_ = ( obj )->binarysink_ )
...
#define put_byte( bs, val )				BinarySink_put_byte( BinarySink_UPCAST( bs ), val )
#define put_uint16( bs, val )			BinarySink_put_uint16( BinarySink_UPCAST( bs ), val )
#define put_uint32( bs, val )			BinarySink_put_uint32( BinarySink_UPCAST( bs ), val )
#define put_uint64( bs, val )			BinarySink_put_uint64( BinarySink_UPCAST( bs ), val )

/* SSH booleans, encoded as a single byte storing either 0 or 1. */
#define put_bool( bs, val )				BinarySink_put_bool( BinarySink_UPCAST( bs ), val )
...
#define put_string( bs, val, len )		BinarySink_put_string( BinarySink_UPCAST( bs ), val, len )
#define put_stringpl( bs, ptrlen )		BinarySink_put_stringpl( BinarySink_UPCAST( bs ), ptrlen )
#define put_stringz( bs, val )			BinarySink_put_stringz( BinarySink_UPCAST( bs ), val )
#define put_stringsb( bs, val )			BinarySink_put_stringsb( BinarySink_UPCAST( bs ), val )
...
#define put_asciz( bs, val )			BinarySink_put_asciz( BinarySink_UPCAST( bs ), val )
#define put_pstring( bs, val )			BinarySink_put_pstring( BinarySink_UPCAST( bs ), val )

/* Multiprecision integers, in the SSH-2 format. */
#define put_mp_ssh2( bs, val )			BinarySink_put_mp_ssh2( BinarySink_UPCAST( bs ), val )

/* Padding with a specified byte. */
#define put_padding( bs, len, padbyte )	BinarySink_put_padding( BinarySink_UPCAST( bs ), len, padbyte )

/* Fallback: just emit raw data bytes, using a syntax that matches the rest of these macros. */
#define put_data( bs, val, len )		BinarySink_put_data( BinarySink_UPCAST( bs ), val, len )
#define put_datapl( bs, pl )			BinarySink_put_datapl( BinarySink_UPCAST( bs ), pl )
...
static __inline void BinarySource_INIT__( BinarySource *src, ptrlen data )
{
	src->data = data.ptr;
	src->len = data.len;
	src->pos = 0;
...
#define BinarySource_BARE_INIT_PL( obj, pl )		TYPECHECK( &( obj )->binarysource_ == ( BinarySource ** )0, BinarySource_INIT__( obj, pl ) )
#define BinarySource_BARE_INIT( obj, data_, len_ )	BinarySource_BARE_INIT_PL( obj, make_ptrlen( data_, len_ ) )
#define BinarySource_INIT_PL( obj, pl )				TYPECHECK( &( obj )->binarysource_ == ( BinarySource ( * )[ 1 ] )0, BinarySource_INIT__( BinarySource_UPCAST( obj ), pl ) )
#define BinarySource_INIT( obj, data_, len_ )		BinarySource_INIT_PL( obj, make_ptrlen( data_, len_ ) )
#define BinarySource_DOWNCAST( object, type )		TYPECHECK( ( object ) == ( ( type * )0 )->binarysource_, ( ( type * )( ( ( char * )( object ) ) - offsetof( type, binarysource_ ) ) ) )
#define BinarySource_UPCAST( object )				TYPECHECK( ( object )->binarysource_ == ( BinarySource * )0, ( object )->binarysource_ )
#define BinarySource_COPIED( obj )					( ( obj )->binarysource_->binarysource_ = ( obj )->binarysource_ )

#define get_data( src, len )				BinarySource_get_data( BinarySource_UPCAST( src ), len )
#define get_byte( src )						BinarySource_get_byte( BinarySource_UPCAST( src ) )
#define get_bool( src )						BinarySource_get_bool( BinarySource_UPCAST( src ) )
#define get_uint16( src )					BinarySource_get_uint16( BinarySource_UPCAST( src ) )
#define get_uint32( src )					BinarySource_get_uint32( BinarySource_UPCAST( src ) )
#define get_uint64( src )					BinarySource_get_uint64( BinarySource_UPCAST( src ) )
#define get_string( src )					BinarySource_get_string( BinarySource_UPCAST( src ) )
#define get_asciz( src )					BinarySource_get_asciz( BinarySource_UPCAST( src ) )
#define get_pstring( src )					BinarySource_get_pstring( BinarySource_UPCAST( src ) )
#define get_mp_ssh2( src )					BinarySource_get_mp_ssh2( BinarySource_UPCAST( src ) )

#define get_err( src )						( BinarySource_UPCAST( src )->err )
#define get_avail( src )					( BinarySource_UPCAST( src )->len - BinarySource_UPCAST( src )->pos )
#define get_ptr( src )						( ( const void * )( ( const unsigned char * )( BinarySource_UPCAST( src )->data ) + BinarySource_UPCAST( src )->pos ) )
