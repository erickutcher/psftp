...
struct BinaryPacketProtocolVtable
{
	...
	PktOut *( *new_pktout )( Ssh *ssh, int type );
	void ( *queue_disconnect )( BinaryPacketProtocol *, const char *msg, int category );
...
static __inline void ssh_bpp_handle_input( BinaryPacketProtocol *bpp )
{ bpp->vt->handle_input( bpp ); }
static __inline void ssh_bpp_handle_output( BinaryPacketProtocol *bpp )
{ bpp->vt->handle_output( bpp ); }
static __inline PktOut *ssh_bpp_new_pktout( Ssh *ssh, BinaryPacketProtocol *bpp, int type )
{ return bpp->vt->new_pktout( ssh, type ); }
static __inline void ssh_bpp_queue_disconnect( BinaryPacketProtocol *bpp, const char *msg, int category )
...
static __inline void dts_consume( struct DataTransferStatsDirection *s, unsigned long size_consumed )
{
	if ( s->running )
	{
...
static __inline void dts_reset( struct DataTransferStatsDirection *s, unsigned long starting_size )
{
	s->expired = false;
...
//BinaryPacketProtocol *ssh2_bare_bpp_new();
...
{
    void ( *got_ssh_version )( struct ssh_version_receiver *rcv, int major_version );
};
