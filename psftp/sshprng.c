...
static void prng_seed_BinarySink_write( BinarySink *bs, const void *data, size_t len );
...
static __inline void prng_generate( prng_impl *pi )
{
...
void prng_add_entropy( prng *pr, unsigned source_id, ptrlen data )
{
	...
	if ( index == 0 )
	{
		pi->until_reseed = ( pi->until_reseed < data.len ? 0 : pi->until_reseed - data.len );
	}

	if ( pi->until_reseed == 0 && prng_reseed_time_ms() - pi->last_reseed_time >= 100 )
	{
		prng_seed_begin( &pi->Prng );
