...
{
	const struct ssh_transient_hostkey_cache_entry *a = ( const struct ssh_transient_hostkey_cache_entry * )av, *b = ( const struct ssh_transient_hostkey_cache_entry * )bv;
...
static int ssh_transient_hostkey_cache_find( void *av, void *bv )
{
	...
	const struct ssh_transient_hostkey_cache_entry *b = ( const struct ssh_transient_hostkey_cache_entry * )bv;
...
void ssh_transient_hostkey_cache_free( ssh_transient_hostkey_cache *thc )
{
	...
	while ( ( ent = ( ssh_transient_hostkey_cache_entry * )delpos234( thc->cache, 0 ) ) != NULL )
	{
...
void ssh_transient_hostkey_cache_add( ssh_transient_hostkey_cache *thc, ssh_key *key )
{
	...
	if ( ( ent = ( ssh_transient_hostkey_cache_entry * )find234( thc->cache, ( void * )ssh_key_alg( key ), ssh_transient_hostkey_cache_find ) ) != NULL )
	{
	...
	retd = ( ssh_transient_hostkey_cache_entry * )add234( thc->cache, ent );
	assert( retd == ent );
}
...
bool ssh_transient_hostkey_cache_verify( ssh_transient_hostkey_cache *thc, ssh_key *key )
{
	...
	if ( ( ent = ( ssh_transient_hostkey_cache_entry * )find234( thc->cache, ( void * )ssh_key_alg( key ), ssh_transient_hostkey_cache_find ) ) != NULL )
	{
		...
		if ( this_blob->len == ent->pub_blob->len && !memcmp( this_blob->s, ent->pub_blob->s, this_blob->len ) )
		{
			toret = true;
		}
...
{
	struct ssh_transient_hostkey_cache_entry *ent = ( ssh_transient_hostkey_cache_entry * )find234( thc->cache, ( void * )alg, ssh_transient_hostkey_cache_find );
	return ent != NULL;
}
