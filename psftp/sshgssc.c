...
static Ssh_gss_stat ssh_gssapi_indicate_mech( struct ssh_gss_library * /*lib*/, Ssh_gss_buf *mech )
{
...
static Ssh_gss_stat ssh_gssapi_import_name( struct ssh_gss_library *lib, char *host, Ssh_gss_name *srv_name )
{
	...
	maj_stat = gss->import_name( &min_stat, &host_buf, GSS_C_NT_HOSTBASED_SERVICE, srv_name );
	/* Release buffer */
	sfree( pStr );
...
static Ssh_gss_stat ssh_gssapi_acquire_cred( struct ssh_gss_library *lib, Ssh_gss_ctx *ctx, time_t *expiry )
{
	struct gssapi_functions *gss = &lib->u.gssapi;
	...
	gssctx->maj_stat = gss->acquire_cred( &gssctx->min_stat, GSS_C_NO_NAME, GSS_C_INDEFINITE, &k5only, GSS_C_INITIATE, &cred, ( gss_OID_set * )0, &time_rec );
	...
	*/
	...
	{
		gssctx->maj_stat = gss->inquire_cred_by_mech( ( OM_uint32 * )&gssctx->min_stat, cred, ( const gss_OID )GSS_MECH_KRB5, ( gss_name_t * )GSS_C_NO_NAME, &time_rec, NULL, NULL );
	}
												 ...
												 unsigned long *lifetime )
{
	...
	if ( gssctx->maj_stat == GSS_S_CONTINUE_NEEDED )
	{
...
static Ssh_gss_stat ssh_gssapi_display_status( struct ssh_gss_library *lib, Ssh_gss_ctx ctx, Ssh_gss_buf *buf )
{
	struct gssapi_functions *gss = &lib->u.gssapi;
	gssapi_ssh_gss_ctx *gssctx = ( gssapi_ssh_gss_ctx * )ctx;
	OM_uint32 lmin,lmax;
...
static Ssh_gss_stat ssh_gssapi_free_tok( struct ssh_gss_library *lib, Ssh_gss_buf *send_tok )
{
	struct gssapi_functions *gss = &lib->u.gssapi;
	OM_uint32 min_stat,maj_stat;
...
static Ssh_gss_stat ssh_gssapi_release_cred( struct ssh_gss_library *lib, Ssh_gss_ctx *ctx )
{
	struct gssapi_functions *gss = &lib->u.gssapi;
	gssapi_ssh_gss_ctx *gssctx = ( gssapi_ssh_gss_ctx * )*ctx;
	OM_uint32 min_stat;
...
static Ssh_gss_stat ssh_gssapi_release_name( struct ssh_gss_library *lib, Ssh_gss_name *srv_name )
{
	struct gssapi_functions *gss = &lib->u.gssapi;
	OM_uint32 min_stat,maj_stat;
...
static Ssh_gss_stat ssh_gssapi_get_mic( struct ssh_gss_library *lib, Ssh_gss_ctx ctx, Ssh_gss_buf *buf, Ssh_gss_buf *hash )
{
	struct gssapi_functions *gss = &lib->u.gssapi;
	gssapi_ssh_gss_ctx *gssctx = ( gssapi_ssh_gss_ctx * )ctx;
	if ( gssctx == NULL )
...
static Ssh_gss_stat ssh_gssapi_verify_mic( struct ssh_gss_library *lib, Ssh_gss_ctx ctx, Ssh_gss_buf *buf, Ssh_gss_buf *hash )
{
	struct gssapi_functions *gss = &lib->u.gssapi;
	gssapi_ssh_gss_ctx *gssctx = ( gssapi_ssh_gss_ctx * )ctx;
	if ( gssctx == NULL )
...
static Ssh_gss_stat ssh_gssapi_free_mic( struct ssh_gss_library *lib, Ssh_gss_buf *hash )
{
