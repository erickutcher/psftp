...
struct PacketProtocolLayerVtable
{
	...
	bool ( *get_specials )( PacketProtocolLayer *ppl, add_special_fn_t add_special, void *ctx );
	void ( *special_cmd )( PacketProtocolLayer *ppl, SessionSpecialCode code, int arg );
	bool ( *want_user_input )( PacketProtocolLayer *ppl );
	void ( *got_user_input )( PacketProtocolLayer *ppl );
	void ( *reconfigure )( PacketProtocolLayer *ppl );
...
static __inline void ssh_ppl_process_queue( PacketProtocolLayer *ppl )
{ ppl->vt->process_queue( ppl ); }
static __inline bool ssh_ppl_get_specials( PacketProtocolLayer *ppl, add_special_fn_t add_special, void *ctx )
{ return ppl->vt->get_specials( ppl, add_special, ctx ); }
static __inline void ssh_ppl_special_cmd( PacketProtocolLayer *ppl, SessionSpecialCode code, int arg )
{ ppl->vt->special_cmd( ppl, code, arg ); }
static __inline bool ssh_ppl_want_user_input( PacketProtocolLayer *ppl )
{ return ppl->vt->want_user_input( ppl ); }
static __inline void ssh_ppl_got_user_input( PacketProtocolLayer *ppl )
{ ppl->vt->got_user_input( ppl ); }
static __inline void ssh_ppl_reconfigure( PacketProtocolLayer *ppl )
{ ppl->vt->reconfigure( ppl ); }
static __inline size_t ssh_ppl_queued_data_size( PacketProtocolLayer *ppl )
...
PacketProtocolLayer *ssh2_transport_new( const char *fullhostname, const char *client_greeting, const char *server_greeting, struct ssh_connection_shared_gss_state *shgss, struct DataTransferStats *stats, PacketProtocolLayer *higher_layer );
PacketProtocolLayer *ssh2_userauth_new( PacketProtocolLayer *successor_layer, const char *fullhostname, Filename *keyfile, const char *default_username, bool try_gssapi_auth, bool try_gssapi_kex_auth, bool gssapi_fwd, struct ssh_connection_shared_gss_state *shgss );
...
void ssh2_transport_provide_hostkeys( PacketProtocolLayer *ssh2_transport_ptr, ssh_key *const *hostkeys, int nhostkeys );
