...
static void arcfour_setkey( ArcfourContext *ctx, unsigned char const *key, unsigned keybytes )
{
...
{
	ArcfourContext *ctx = container_of( cipher, ArcfourContext, ciph );
	arcfour_setkey( ctx, ( const unsigned char * )key, ctx->ciph.vt->padded_keybytes );
	arcfour_stir( ctx );
}
