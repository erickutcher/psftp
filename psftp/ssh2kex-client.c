...
void ssh2kex_coroutine( struct ssh2_transport_state *s, bool *aborted )
{
	...
	if ( s->kex_alg->main_type == KEXTYPE_DH )
	{
		...
		*/
		...
		/* The keys only have hlen-bit entropy, since they're based on a hash. So cap the key size at hlen bits. */
		...
		/*
		* If we're doing Diffie-Hellman group exchange, start by requesting a group.
		*/
		if ( dh_is_gex( s->kex_alg ) )
		{
//			s->ppl.bpp->pls->kctx = SSH2_PKTCTX_DHGEX;
			/*
			* Work out how big a DH group we will need to allow that much data.
			*/
			...
			{
				pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, SSH2_MSG_KEX_DH_GEX_REQUEST_OLD );
				put_uint32( pktout, s->pbits );
			}
			else
			{
				pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, SSH2_MSG_KEX_DH_GEX_REQUEST );
...
//			s->ppl.bpp->pls->kctx = SSH2_PKTCTX_DHGROUP;
		...
		*/
		s->e = dh_create_e( s->ppl.ssh->pr, s->dh_ctx, s->nbits * 2 );
		pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, s->kex_init_value );
...
//		s->ppl.bpp->pls->kctx = SSH2_PKTCTX_ECDHKEX;
		...
		s->ecdh_key = ssh_ecdhkex_newkey( s->ppl.ssh->pr, s->kex_alg );
		if ( !s->ecdh_key )
		{
		...
		pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, SSH2_MSG_KEX_ECDH_INIT );
		{
		...
		*/
		...
		if ( dh_is_gex( s->kex_alg ) )
		{
			...
			*/
			...
			pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, SSH2_MSG_KEXGSS_GROUPREQ );
		...
		s->gss_stat = s->shgss->lib->acquire_cred( s->shgss->lib, &s->shgss->ctx, &s->gss_cred_expiry );
		...
		do
		{
			...
			*/
			s->gss_stat = s->shgss->lib->init_sec_context( s->shgss->lib, &s->shgss->ctx, s->shgss->srv_name, s->gss_delegate, &s->gss_rcvtok, &s->gss_sndtok, ( s->gss_cred_expiry == GSS_NO_EXPIRATION ? &s->gss_cred_expiry : NULL ), NULL );
			SSH_GSS_CLEAR_BUF( &s->gss_rcvtok );
			...
			assert( s->gss_stat == SSH_GSS_S_COMPLETE || s->gss_stat == SSH_GSS_S_CONTINUE_NEEDED );

			if ( !s->init_token_sent )
			{
				s->init_token_sent = true;
				pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, SSH2_MSG_KEXGSS_INIT );
				...
				put_string( pktout, s->gss_sndtok.value, s->gss_sndtok.length );
				put_mp_ssh2( pktout, s->e );
				pq_push( s->ppl.ssh, s->ppl.out_pq, pktout );
			...
			{
				pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, SSH2_MSG_KEXGSS_CONTINUE );
				put_string( pktout, s->gss_sndtok.value, s->gss_sndtok.length );
				pq_push( s->ppl.ssh, s->ppl.out_pq, pktout );
				s->shgss->lib->free_tok( s->shgss->lib, &s->gss_sndtok );
			}
			...
			switch ( pktin->type )
			{
				...
				case SSH2_MSG_KEXGSS_HOSTKEY:
				{
					...
					/*
					* Can't loop as we have no token to pass to init_sec_context.
					*/
		...
		while ( s->gss_rcvtok.length || s->gss_stat == SSH_GSS_S_CONTINUE_NEEDED || !s->complete_rcvd );
		...
		/*
		* MIC verification is done below, after we compute the hash used as the MIC input.
		*/
...
//		s->ppl.bpp->pls->kctx = SSH2_PKTCTX_RSAKEX;
		/*
		* RSA key exchange. First expect a KEXRSA_PUBKEY packet from the server.
		*/
		crMaybeWaitUntilV( ( pktin = ssh2_transport_pop( s ) ) != NULL );
		...
		*/
		{
			...
			mp_int *tmp = mp_random_bits( s->ppl.ssh->pr, nbits - 1 );
			...
			*/
			outstr = ssh_rsakex_encrypt( s->ppl.ssh->pr, s->rsa_kex_key, s->kex_alg->hash, ptrlen_from_strbuf( buf ) );

			/*
			* And send it off in a return packet.
			*/
			pktout = ssh_bpp_new_pktout( s->ppl.ssh, s->ppl.bpp, SSH2_MSG_KEXRSA_SECRET );
	...
	if ( s->kex_alg->main_type == KEXTYPE_GSS )
	{
		...
		s->gss_stat = s->shgss->lib->verify_mic( s->shgss->lib, s->shgss->ctx, &gss_buf, &s->mic );
		if ( s->gss_stat != SSH_GSS_OK )
		{
			if ( s->shgss->lib->display_status( s->shgss->lib, s->shgss->ctx, &s->gss_buf ) == SSH_GSS_OK )
			{
	...
	if ( s->kex_alg->main_type != KEXTYPE_GSS )
	{
		...
		if ( !ssh_key_verify( s->hkey, s->sigdata, make_ptrlen( s->exchange_hash, s->kex_alg->hash->hlen ) ) )
		{
#ifndef FUZZING
			ssh_proto_error(s->ppl.ssh );
			*aborted = true;
	...
	{
		/*
		* In a GSS-based session, check the host key (if any) against the transient host key cache.
		*/
		if ( s->kex_alg->main_type == KEXTYPE_GSS )
		{
			/*
			* We've just done a GSS key exchange. If it gave us a host key, store it.
			*/
			if ( s->hkey )
			{
		...
		if ( !s->got_session_id )
	{
		// Assume no key found in cache.

		get_key_info( s->hkey, &s->ppl.ssh->key_algorithm, &s->ppl.ssh->key_size, &s->ppl.ssh->md5_key_fingerprint, &s->ppl.ssh->sha256_key_fingerprint );

		if ( s->ppl.ssh->key_info == NULL || s->ppl.ssh->key_info_count == 0 )
		{
			s->ppl.ssh->ssh_status |= SSH_STATUS_KEY_NOT_FOUND;
		}
		else
		{
			bool fingerprint_match = false;

			//char *algorithm = ( char * )ssh_key_ssh_id( s->hkey );

			for ( unsigned int i = 0; i < s->ppl.ssh->key_info_count; ++i )
			{
				if ( strcmp( s->ppl.ssh->key_algorithm, s->ppl.ssh->key_info[ i ].algorithm ) == 0 &&
				   ( strcmp( s->ppl.ssh->sha256_key_fingerprint, s->ppl.ssh->key_info[ i ].fingerprint ) == 0 ||
					 strcmp( s->ppl.ssh->md5_key_fingerprint, s->ppl.ssh->key_info[ i ].fingerprint ) == 0 ) )
				{
			...
			{
				s->ppl.ssh->ssh_status |= SSH_STATUS_KEY_MISMATCH;
			}
		}

		/*
		* Save this host key, to check against the one presented in subsequent rekeys.
		*/
	...
	else if ( s->cross_certifying )
	{
		...
		/*
		* Don't forget to store the new key as the one we'll be re-checking in future normal rekeys.
		*/
		s->hostkey_str = s->keystr;
		s->keystr = NULL;
	}
