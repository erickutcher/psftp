...
static void ssh2_mac_prepare( ssh2_mac *mac, const void *blk, int len, unsigned long seq )
{
	ssh2_mac_start( mac );
	put_uint32( mac, seq );
...
bool ssh2_mac_verify( ssh2_mac *mac, const void *blk, int len, unsigned long seq )
{
	ssh2_mac_prepare( mac, blk, len, seq );
