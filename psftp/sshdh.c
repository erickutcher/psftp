...
{
	if ( ctx->x ) { mp_free( ctx->x ); }
	if ( ctx->e ) { mp_free( ctx->e ); }
	if ( ctx->p ) { mp_free( ctx->p ); }
	if ( ctx->g ) { mp_free( ctx->g ); }
	if ( ctx->q ) { mp_free( ctx->q ); }
    sfree( ctx );
}
 ...
 */
mp_int *dh_create_e( prng *pr, dh_ctx *ctx, int nbits )
{
	/*
	...
	*/
	ctx->x = mp_random_in_range( pr, lo, hi );
	mp_free( lo );
	mp_free( hi );
