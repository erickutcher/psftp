...
static WeierstrassPoint *ecc_weierstrass_point_new_imported( WeierstrassCurve *wc, mp_int *monty_x, mp_int *monty_y )
{
...
WeierstrassPoint *ecc_weierstrass_point_new( WeierstrassCurve *wc, mp_int *x, mp_int *y )
{
    return ecc_weierstrass_point_new_imported( wc, monty_import( wc->mc, x ), monty_import( wc->mc, y ) );
}
...
void ecc_weierstrass_point_copy_into( WeierstrassPoint *dest, WeierstrassPoint *src )
{
...
static void ecc_weierstrass_cond_overwrite( WeierstrassPoint *dest, WeierstrassPoint *src, unsigned overwrite )
{
...
static void ecc_weierstrass_cond_swap( WeierstrassPoint *P, WeierstrassPoint *Q, unsigned swap )
{
 ...
 */
static __inline void ecc_weierstrass_epilogue( mp_int *Px, mp_int *Qx, mp_int *Py, mp_int *common_Z, mp_int *lambda_n, mp_int *lambda_d, WeierstrassPoint *out )
{
 ...
 * denominator.
 */
static __inline void ecc_weierstrass_add_prologue( WeierstrassPoint *P, WeierstrassPoint *Q, mp_int **Px, mp_int **Py, mp_int **Qx, mp_int **denom, mp_int **lambda_n, mp_int **lambda_d )
{
 ...
 */
static __inline void ecc_weierstrass_tangent_slope( WeierstrassPoint *P, mp_int **lambda_n, mp_int **lambda_d )
{
	WeierstrassCurve *wc = P->wc;
...
static __inline void ecc_weierstrass_select_into( WeierstrassPoint *dest, WeierstrassPoint *P, WeierstrassPoint *Q, unsigned choose_Q )
{
...
void ecc_montgomery_point_copy_into( MontgomeryPoint *dest, MontgomeryPoint *src )
{
    mp_copy_into( dest->X, src->X );
    mp_copy_into( dest->Z, src->Z );
}
...
static void ecc_montgomery_cond_overwrite( MontgomeryPoint *dest, MontgomeryPoint *src, unsigned overwrite )
{
	mp_select_into( dest->X, dest->X, src->X, overwrite );
	mp_select_into( dest->Z, dest->Z, src->Z, overwrite );
}

static void ecc_montgomery_cond_swap( MontgomeryPoint *P, MontgomeryPoint *Q, unsigned swap )
{
	mp_cond_swap( P->X, Q->X, swap );
	mp_cond_swap( P->Z, Q->Z, swap );
}

MontgomeryPoint *ecc_montgomery_diff_add( MontgomeryPoint *P, MontgomeryPoint *Q, MontgomeryPoint *PminusQ )
{
...
static EdwardsPoint *ecc_edwards_point_new_imported( EdwardsCurve *ec, mp_int *monty_x, mp_int *monty_y )
{
...
EdwardsPoint *ecc_edwards_point_new( EdwardsCurve *ec, mp_int *x, mp_int *y )
{
	return ecc_edwards_point_new_imported( ec, monty_import( ec->mc, x ), monty_import( ec->mc, y ) );
}
...
EdwardsPoint *ecc_edwards_point_new_from_y( EdwardsCurve *ec, mp_int *yorig, unsigned desired_x_parity )
{
	assert( ec->sc );
...
static void ecc_edwards_cond_overwrite( EdwardsPoint *dest, EdwardsPoint *src, unsigned overwrite )
{
	mp_select_into( dest->X, dest->X, src->X, overwrite );
	mp_select_into( dest->Y, dest->Y, src->Y, overwrite );
	mp_select_into( dest->Z, dest->Z, src->Z, overwrite );
...
static void ecc_edwards_cond_swap( EdwardsPoint *P, EdwardsPoint *Q, unsigned swap )
{
	mp_cond_swap( P->X, Q->X, swap );
	mp_cond_swap( P->Y, Q->Y, swap );
	mp_cond_swap( P->Z, Q->Z, swap );
 ...
 */
static __inline unsigned projective_eq( MontyContext *mc, mp_int *An, mp_int *Ad, mp_int *Bn, mp_int *Bd )
{
