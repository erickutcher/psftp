...
{
	const ssh_hashalg *real_alg = sha256_hw_available_cached() ? &ssh_sha256_hw : &ssh_sha256_sw;
...
static __inline void sha256_block_setup( sha256_block *blk )
{
	blk->used = 0;
	blk->len = 0;
}

static __inline bool sha256_block_write( sha256_block *blk, const void **vdata, size_t *len )
{
	size_t blkleft = sizeof( blk->block ) - blk->used;
	size_t chunk = *len < blkleft ? *len : blkleft;

	const uint8_t *p = ( const uint8_t * )*vdata;
...
static __inline void sha256_block_pad( sha256_block *blk, BinarySink *bs )
{
...
static __inline uint32_t ror( uint32_t x, unsigned y )
{
	return ( x << ( 31 & -y ) ) | ( x >> ( 31 & y ) );
}

static __inline uint32_t Ch( uint32_t ctrl, uint32_t if1, uint32_t if0 )
{
	return if0 ^ ( ctrl & ( if1 ^ if0 ) );
}

static __inline uint32_t Maj( uint32_t x, uint32_t y, uint32_t z )
{
	return ( x & y ) | ( z & ( x | y ) );
}

static __inline uint32_t Sigma_0( uint32_t x )
{
	return ror( x, 2 ) ^ ror( x, 13 ) ^ ror( x, 22 );
}

static __inline uint32_t Sigma_1( uint32_t x )
{
...
static __inline void sha256_sw_round( unsigned round_index, const uint32_t *schedule,
									  uint32_t *a, uint32_t *b, uint32_t *c, uint32_t *d,
									  uint32_t *e, uint32_t *f, uint32_t *g, uint32_t *h )
{
	uint32_t t1 = *h + Sigma_1( *e ) + Ch( *e, *f, *g ) + sha256_round_constants[ round_index ] + schedule[ round_index ];
...
#if defined(__clang__) || defined(__GNUC__)
#include <cpuid.h>
	#define GET_CPU_ID_0( out )	__cpuid( 0, ( out )[ 0 ], ( out )[ 1 ], ( out )[ 2 ], ( out )[ 3 ] )
	#define GET_CPU_ID_7( out )	__cpuid_count( 7, 0, ( out )[ 0 ], ( out )[ 1 ], ( out )[ 2 ], ( out )[ 3 ] )
#else
...
static bool sha256_hw_available( void )
{
	...
	GET_CPU_ID_0( ( int * )CPUInfo );
	if ( CPUInfo[ 0 ] < 7 )
	{
		return false;
	}

	GET_CPU_ID_7( ( int * )CPUInfo );
...
*/
FUNC_ISA
static __inline void sha256_ni_block( __m128i *core, const uint8_t *p )
{
...
FUNC_ISA static ssh_hash *sha256_ni_new( const ssh_hashalg *alg )
{
	...
	s->core[ 0 ] = _mm_set_epi64x( 0x6a09e667bb67ae85ULL, 0x510e527f9b05688cULL );
	s->core[ 1 ] = _mm_set_epi64x( 0x3c6ef372a54ff53aULL, 0x1f83d9ab5be0cd19ULL );
...
#define STUB_BODY_RET { /*unreachable( "Should never be called" );*/ return NULL; }

static ssh_hash *sha256_stub_copy( ssh_hash * /*hash*/ ) STUB_BODY_RET
...
const ssh_hashalg ssh_sha256_hw =
{
	...
	32, 64, HASHALG_NAMES_ANNOTATED( "SHA-256", "!NONEXISTENT ACCELERATED VERSION!" ),
};
