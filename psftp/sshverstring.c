...
static PktOut *ssh_verstring_new_pktout( Ssh *ssh, int type );
static void ssh_verstring_queue_disconnect( BinaryPacketProtocol *bpp, const char *msg, int category );
...
BinaryPacketProtocol *ssh_verstring_new( const char *protoversion, struct ssh_version_receiver *rcv, bool server_mode, const char *impl_name )
{
...
{
	struct ssh_verstring_state *s = container_of( bpp, struct ssh_verstring_state, bpp );
	sfree( s->impl_name );
...
static void ssh_verstring_send( struct ssh_verstring_state *s )
{
	...
	*/
	s->our_vstring = dupprintf( "%.*s%s-%s%s", ( int )s->prefix_wanted.len, ( const char * )s->prefix_wanted.ptr, s->our_protoversion, s->impl_name, sshver );
	sv_pos = s->prefix_wanted.len + strlen( s->our_protoversion ) + 1;

	/* Convert minus signs and spaces in the software version string into underscores. */
	...
	*/
	bufchain_add( s->bpp.ssh, s->bpp.out_raw, s->our_vstring, strlen( s->our_vstring ) );
	if ( ssh_version_includes_v2( s->our_protoversion ) )
	{
		bufchain_add( s->bpp.ssh, s->bpp.out_raw, "\015", 1 );
	}
	bufchain_add( s->bpp.ssh, s->bpp.out_raw, "\012", 1 );
}
...
#define BPP_WAITFOR( minlen )	\
	...
	{							\
		bool success;			\
		crMaybeWaitUntilV( ( success = ( bufchain_size( s->bpp.in_raw ) >= ( minlen ) ) ) || s->bpp.input_eof );	\
		if ( !success )			\
		{						\
			goto eof;			\
		}						\
...
void ssh_verstring_handle_input( BinaryPacketProtocol *bpp )
{
	...
	/*
	* If we're sending our version string up front before seeing the other side's, then do it now.
	*/
	if ( s->send_early )
	{
	...
	/*
	* Search for a line beginning with the protocol name prefix in the input.
	*/
	s->i = 0;
	while ( 1 )
	{
		/*
		* Every time round this loop, we're at the start of a new line, so look for the prefix.
		*/
		...
		*/
		while ( 1 )
		{
			...
			/* Wait to receive at least 1 byte, but then consume more than that if it's there. */
			BPP_WAITFOR( 1 );
			data = bufchain_prefix( s->bpp.in_raw );
			if ( ( nl = ( char * )memchr( data.ptr, '\012', data.len ) ) != NULL )
			{
	...
	*/
	...
	/*
	* Trim \r and \n from the version string, and replace them with a NUL terminator.
	*/
	while ( s->vstring->len > 0 && ( s->vstring->s[ s->vstring->len - 1 ] == '\r' || s->vstring->s[ s->vstring->len - 1 ] == '\n' ) )
	{
	...
	*/
	if ( ssh_version_includes_v2( s->our_protoversion ) && ssh_version_includes_v2( s->protoversion ) )
	{
		/*
		* We're doing SSH-2.
		*/
	...
	else if ( ssh_version_includes_v1( s->our_protoversion ) && ssh_version_includes_v1( s->protoversion ) )
	{
		/*
		* We're doing SSH-1.
		*/
		...
		* the other side.
		*/
		if ( !s->send_early && ssh_versioncmp( s->our_protoversion, s->protoversion ) > 0 )
		{
			sfree( s->our_protoversion );
...
static PktOut *ssh_verstring_new_pktout( Ssh * /*ssh*/, int /*type*/ )
{
//	unreachable( "Should never try to send packets during SSH version string exchange" );
	return NULL;
}

static void ssh_verstring_handle_output( BinaryPacketProtocol *bpp )
{
	if ( pq_peek( bpp->ssh, &bpp->out_pq ) )
	{
//		unreachable( "Should never try to send packets during SSH version string exchange" );
	}
}
...
/*
 * Examine the remote side's version string, and compare it against a list of known buggy implementations.
 */
static void ssh_detect_bugs( struct ssh_verstring_state *s )
{
	...
	*/
	if ( g_CONF_sshbug_ignore1 == FORCE_ON ||
	   ( g_CONF_sshbug_ignore1 == AUTO &&
	...
	if ( g_CONF_sshbug_plainpw1 == FORCE_ON ||
	   ( g_CONF_sshbug_plainpw1 == AUTO &&
	...
	if ( g_CONF_sshbug_rsa1 == FORCE_ON ||
	   ( g_CONF_sshbug_rsa1 == AUTO &&
	...
	if ( g_CONF_sshbug_hmac2 == FORCE_ON ||
	   ( g_CONF_sshbug_hmac2 == AUTO &&
		!wc_match( "* VShell", imp ) &&
	...
	if ( g_CONF_sshbug_derivekey2 == FORCE_ON ||
	   ( g_CONF_sshbug_derivekey2 == AUTO &&
		!wc_match( "* VShell", imp ) &&
	...
	if ( g_CONF_sshbug_rsapad2 == FORCE_ON ||
	   ( g_CONF_sshbug_rsapad2 == AUTO &&
	...
	if ( g_CONF_sshbug_pksessid2 == FORCE_ON ||
	   ( g_CONF_sshbug_pksessid2 == AUTO &&
	...
	if ( g_CONF_sshbug_rekey2 == FORCE_ON ||
	   ( g_CONF_sshbug_rekey2 == AUTO &&
	...
	if ( g_CONF_sshbug_maxpkt2 == FORCE_ON ||
	   ( g_CONF_sshbug_maxpkt2 == AUTO &&
	...
	if ( g_CONF_sshbug_ignore2 == FORCE_ON )
	{
		/*
	...
	if ( g_CONF_sshbug_oldgex2 == FORCE_ON ||
	   ( g_CONF_sshbug_oldgex2 == AUTO &&
	...
	if ( g_CONF_sshbug_winadj == FORCE_ON)
	{
		/*
	...
	if ( g_CONF_sshbug_chanreq == FORCE_ON ||
	   ( g_CONF_sshbug_chanreq == AUTO &&
