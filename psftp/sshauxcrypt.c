...
void des3_decrypt_pubkey_ossh( const void *vkey, const void *viv, void *vblk, int len )
{
...
void des3_encrypt_pubkey_ossh( const void *vkey, const void *viv, void *vblk, int len )
{
	ssh_cipher *c = des3_pubkey_ossh_cipher( vkey, viv );
	ssh_cipher_encrypt( c, vblk, len );
	ssh_cipher_free( c );
...
void hash_simple( const ssh_hashalg *alg, ptrlen data, void *output )
{
	...
	ssh_hash_final( hash, ( unsigned char * )output );
}
...
void mac_simple( const ssh2_macalg *alg, ptrlen key, ptrlen data, void *output )
{
	...
	ssh2_mac_start( mac );
	put_datapl( mac, data );
	ssh2_mac_genresult( mac, ( unsigned char * )output );
	ssh2_mac_free( mac );
}
